...
- Balayage des 41 paquets : 37 mesurés verts ; `zcrud_generator` rouge environnemental ; **quatre paquets pur-Dart non remesurés en fin de chaîne** à cause d'une coupure réseau (`dart test` appelle `pub.dev` avant de lancer) — `zcrud_chat_kernel` mesuré vert en tête de chaîne, `zcrud_exam`, `zcrud_chat_syncfusion`, `zcrud_study_kernel` **identiques à l'octet à v3.8.0** (`git diff --quiet v3.8.0`), balayée verte.
