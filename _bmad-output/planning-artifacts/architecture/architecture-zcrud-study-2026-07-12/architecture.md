...
- 🟡 **DW-ES94-1 — l'ENFORCEMENT SERVEUR de l'ACL de partage est HORS DOMAINE (story ES-9.4, 2026-07-16).** La dette de sécurité héritée de lex (un contributeur pouvant réécrire des champs de contrôle ; limite LWW/révocation à la prochaine sync) est **CORRIGÉE PAR CONCEPTION côté domaine** : les champs de contrôle vivent dans des entités owner-contrôlées, la garde PURE `ZStudySharingAcl.canMutateControl(role, actorUid, ownerUid)` retourne `false` pour tout non-owner (contributeur/viewer/inconnu), la révocation est monotone (un non-owner ne peut dé-révoquer), et l'état personnel (`ZRepetitionInfo`/`ZFolderContentsOrder`/lecture-doc) est **structurellement séparé** du sous-arbre partageable (intersection de clés VIDE). Ces gardes sont **verrouillées en machine** (`z_study_sharing_acl_test.dart` rougit si `canMutateControl` cède ; `z_study_sharing_personal_state_test.dart` rougit si un champ SRS entre dans une entité de partage ; spot-check orchestrateur R3-ACL confirmé RC=1). **RÉSIDUEL À SOLDER par l'app (AD-11/AD-12/AD-26)** : le domaine étant backend-agnostique, il ne peut pas EMPÊCHER une écriture de contrôle **forgée** ni le résiduel LWW distant — l'**enforcement serveur** (règles backend rejetant à la source, `role` vérifié côté store et non fourni par l'appelant) doit **RÉPLIQUER** le prédicat pur `canMutateControl`. Documenté en **dartdoc impossible à rater** dans `z_study_sharing_acl.dart` + les ports. **Signalé, jamais hérité en silence** (NFR-S11) : le domaine fournit le prédicat de vérité, l'app le fait respecter au serveur.
