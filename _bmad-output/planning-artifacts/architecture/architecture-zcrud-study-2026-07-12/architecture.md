...
- 🔴 **DW-ES22-1 — `normalizeNoteContentOps` (`zcrud_note`) et `DeltaNeutralOps.asDeltaOps` (`zcrud_markdown`) ne sont pas « redondantes » : elles DIVERGENT, EN SENS OPPOSÉ SUR LA DONNÉE. ⛔ À RÉCONCILIER AVANT ES-6.1.**
  ...
  - **Correctif de fond** : hisser la primitive **préservante** dans `zcrud_core` (pur-Dart) et faire consommer `zcrud_markdown` ⇒ **écrit `zcrud_core`** ⇒ hors périmètre ES-2 (**D9**). **Critère de clôture** : une **seule** implémentation, et un test exécutant les deux voies sur `'# T'` avec le **même** résultat **préservant**.
