...
- **Story E3-3c. Champ fichier / image / document (`ZAppFileField`).** En tant que dev, je peux déclarer un champ fichier/image/document. AC : `AppFile` + `FileFieldConfig` + `ZAppFileField` (scan/caméra/galerie/picker, prévisualisation) dans `zcrud_core`, derrière le port `CloudStorageRepository` ; l'upload/download concret est fourni par l'app (impl Firebase Storage en `zcrud_firestore`, E5) ; **couvre les types `file`/`image`/`document`** du catalogue (parité DODLP, SM-2) ; a11y/RTL comme E3-3a. *(Comble le trou de couverture relevé au contrôle de complétude.)*
...
**Objectif :** adaptateur Firestore débogué + patron offline-first. **Couvre :** FR-12, FR-13 · AD-5, AD-9, AD-11. **Dépend de :** E2 · **Phase :** E5-1/E5-2 = **MVP** (repo Firestore + `ZLocalStore`, requis par E7/E8) ; **E5-3/E5-4 (offline-first LWW + orchestrateur) = v1.x** — leur consommateur est la donnée d'étude (E9) ; aucun écran MVP ne les requiert (harnais de validation livré avec E9-4).
