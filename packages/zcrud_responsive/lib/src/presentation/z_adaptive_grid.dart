...
///
/// **Nom `ZAdaptiveGrid` — JAMAIS `ZResponsiveGrid`**: `ZResponsiveGrid`
...
///
/// **Largeur LOCALE**: `availableWidth = constraints.maxWidth` du
...
/// zéro.
///
/// **Aspect déduit**: les items s'étirent pour remplir la largeur (largeur
...
///
/// **Rendu paresseux**: `GridView.builder` (`itemCount` +
...
///
/// ## Deux constructeurs, une seule mécanique
///
...
/// * [ZAdaptiveGrid.builder] (`itemCount`/`itemBuilder`), pour les
///   collections **volumineuses** (des milliers de cartes) : la grille
///   **scrolle d'elle-même** et ne construit que le viewport.
///
...
class ZAdaptiveGrid extends StatelessWidget {
  ...
  ///
  /// **EAGER**: [children] est matérialisé en entier par l'appelant, et la
  /// grille layoute **tout** (`shrinkWrap`, parent scrollable). Pour une
  /// collection volumineuse, préférer [ZAdaptiveGrid.builder].
  const ZAdaptiveGrid({
  ...
  /// Crée une grille adaptative **VIRTUALISÉE**.
  ///
  ...
  ///. Privé: le mode est un détail d'implémentation, la surface
  ...
  /// (`largeur locale / minItemWidth`, borné) — remplace un seuil codé en
  /// dur figé.
  ...
  /// réallouée à chaque build (stabilité d'identité, invariant AD-2).
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        // Padding horizontal résolu **directionnellement** (invariant AD-13): il est
        ...
        return GridView.builder(
          // La seule différence entre les deux modes.
          ...
          //   cull réellement les items. `shrinkWrap: true` ici
