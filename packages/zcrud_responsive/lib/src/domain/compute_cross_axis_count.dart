...
///
/// **Fonction pure de domaine** (top-level, aucun `BuildContext`, aucun
/// import Flutter — au plus `dart:math`) : testable en `test/` **sans
/// surface widget**, déterministe (invariant AD-14).
///
/// **Formule** : `n = ⌊(effectiveWidth + spacingEff) / (minItemWidth +
/// spacingEff)⌋`, borné par `clamp(lo, hi)` où `lo = max(1, minColumns)`,
/// `hi = maxColumns ?? +∞`, `effectiveWidth = availableWidth −
/// horizontalPadding` et `spacingEff` = [spacing] retenu s'il est fini et
/// `> 0`, sinon `0`. Ainsi chaque item fait **au moins** [minItemWidth]
/// **une fois les `n − 1` gouttières de largeur [spacing] et le
/// [horizontalPadding] déduits** — jamais de surestimation qui écraserait
/// les items sous [minItemWidth].
///
/// **Rétro-compatibilité** : avec `spacing == 0` **et**
/// `horizontalPadding == 0` (valeurs par défaut), la formule se réduit
/// **exactement** à `(availableWidth / minItemWidth).floor()`.
///
/// **Garantie `≥ 1`** : le plancher `lo ≥ 1` est **toujours** appliqué —
/// pour `availableWidth < minItemWidth`, le résultat est `1` (jamais `0`,
/// donc jamais de division par zéro en aval au calcul de la largeur d'item
/// ni de `GridView` vide).
///
/// **Défauts sûrs (invariant AD-10) — jamais de `throw`, jamais de division
/// par zéro** :
/// * `minItemWidth <= 0` ou `NaN` → `lo` (aucune division n'est tentée) ;
/// * `availableWidth <= 0` ou `NaN` → `lo` ;
/// * `availableWidth` **infini** → `maxColumns` (remonté à `lo` si `< lo`)
///   s'il est fourni, sinon `lo` (jamais de grille non bornée) ;
...
/// * `maxColumns` fourni `< lo` → **remonté** à `lo` (le `clamp` reste
///   toujours valide, `lo <= hi` — jamais de `RangeError`/`ArgumentError`).
int computeCrossAxisCount({
  ...
  // Plancher garanti : jamais moins de 1 colonne.
  ...
  // Gouttière & padding assainis: négatif / NaN / infini → 0.
  ...
  // Largeur utile après retrait du padding horizontal.
