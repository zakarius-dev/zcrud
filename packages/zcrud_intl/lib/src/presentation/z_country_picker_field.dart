...
/// invariants AD-2, AD-13).
///
/// Le champ pays, le champ téléphone (indicatif) et le sous-formulaire
/// adresse ont tous besoin de choisir un pays depuis le [ZCountryCatalog].
/// Ce widget factorise ce comportement : une **cible tactile ≥48 dp**
/// (drapeau + nom ou indicatif) qui déplie un **panneau de recherche +
/// liste** (`ListView.builder`) alimenté paresseusement par le catalogue.
///
/// **Invariant AD-2** : `TextEditingController`/`FocusNode` de recherche
/// créés **1×** (`initState`), disposés en `dispose`, jamais recréés. Le
/// chargement paresseux du catalogue déclenche un `setState` **local**
/// (rebuild ciblé de ce champ uniquement, jamais du formulaire).
///
/// **État d'affichage** : l'état « déplié » accepte un
/// [ZCountryPickerField.openController] **optionnel**. Sans lui,
...
class _ZCountryPickerFieldState extends State<ZCountryPickerField> {
  ...
  void initState() {
    ...
    // résolu, on le charge puis on rebuild CE champ (état local, sans impact sur le reste de l'arbre).
  ...
  Widget _trigger(ZcrudTheme theme, ZCountryInfo? selected) {
    ...
    return Semantics(
      ...
      label: semLabel,
      value: display,
      // Opérabilité (invariant AD-13) : câbler l'action de tap SUR le nœud
      // sémantique englobant — sinon `ExcludeSemantics` retire l'action
      // native de l'`InkWell` et le lecteur d'écran ne peut PAS déclencher
      // l'ouverture.
  ...
  Widget _triggerRow(
    ...
    // Aucune couleur en dur — jeton du thème hors décoration, rôles
    // Material dans le cadre décoré (parité cœur).
          ...
          itemBuilder: (context, i) {
            ...
            return ConstrainedBox(
              ...
              child: Semantics(
                ...
                // Action de sélection portée par le nœud sémantique
                // englobant (opérable au lecteur d'écran malgré
                // ExcludeSemantics).
                onTap: () => _select(c),
                child: ExcludeSemantics(
                  child: ListTile(
