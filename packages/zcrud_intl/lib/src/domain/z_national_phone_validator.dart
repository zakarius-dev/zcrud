...
/// (invariants AD-1, AD-10, AD-12, AD-14).
///
/// Une politique téléphonique **nationale** (longueur fixe + liste de
/// préfixes autorisés, par pays) n'est pas capturée par le seul
/// [ZPhoneCodec] E.164 générique. Plutôt qu'étendre le catalogue **fermé**
/// `ZValidatorKind` du cœur (édition interdite à un satellite, invariant
/// AD-1), ce validateur **autonome, pur-Dart** vit dans `zcrud_intl`,
/// **paramétrable** ([prefixes] + [length]) et **orthogonal** au codec
/// (opt-in, n'altère jamais la valeur émise).
///
/// **Neutralité (invariant AD-12)** : **aucun** pays, préfixe ni longueur
/// codé en dur. La politique nationale est fournie **par paramètres**
/// (recette ci-dessous), jamais en défaut du package.
///
/// **Défensif (invariant AD-10)** : [validate] **ne throw jamais** —
/// accepte `null`, un [ZPhoneNumber], une `Map` sérialisée ou une `String`
/// brute, et normalise défensivement toute entrée inattendue.
///
/// **Pur-Dart (invariant AD-14)** : n'importe **ni** Flutter, **ni**
/// `phone_numbers_parser`, **ni** `zcrud_core` — seule dépendance : le
/// modèle neutre [ZPhoneNumber].
///
/// ## Exemple de recette — neutre et surchargeable
///
/// Une politique nationale typique : longueur fixe + préfixe ∈ un
/// ensemble donné (ici, huit chiffres et un préfixe parmi
/// `{70,71,77,78,79,90,91,92,93,96,97,98,99}`, comme au Togo). Selon la
/// cible, on valide soit la chaîne nationale **formatée**
/// (`"90 12 34 56"` = **11** caractères, espaces compris), soit les
/// **chiffres nus** exposés par [ZPhoneNumber.nationalNumber] (**8**
/// chiffres pour cet exemple). Deux politiques équivalentes selon la
/// cible :
///
/// ```dart
...
/// );
///
/// // Variante B — sur la chaîne nationale formatée ("90 12 34 56") :
...
/// Discriminant d'erreur **neutre** (sans message) — la traduction vit dans
/// la couche présentation (`nationalPhoneErrorText`, invariants AD-1,
/// AD-13).
...
///
/// Un numéro dont la partie nationale a la **bonne [length]** ET commence
/// par **l'un des [prefixes]** est **valide** ([validate] renvoie `null`) ;
/// sinon [validate] renvoie le [ZNationalPhoneError] discriminant, évalué
/// dans l'ordre requis → longueur → préfixe.
