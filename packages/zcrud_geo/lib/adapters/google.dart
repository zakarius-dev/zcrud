...
///
/// **Invariant AD-1 (confinement SDK)** : le SDK Google n'est atteignable que
/// par cet import explicite (`package:zcrud_geo/adapters/google.dart`),
/// jamais par le barrel principal `package:zcrud_geo/zcrud_geo.dart`. Un
/// consommateur qui n'a pas besoin de Google Maps (ou fournit son propre
/// adaptateur / utilise OSM) ne référence aucun symbole
/// `google_maps_flutter` dans son code. La dépendance reste néanmoins
/// déclarée au `pubspec.yaml` de `zcrud_geo` (jamais de `zcrud_core`).
///
/// **Invariant AD-12 (aucune clé dans le package)** : la clé API Google Maps
/// vit dans la **config plateforme** de l'application hôte — jamais dans
/// `zcrud_geo` :
