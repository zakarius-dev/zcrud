/// Migrateur de **CORPUS** de documents d'étude Firestore legacy → forme
/// CANONIQUE zcrud (invariants AD-10/AD-5).
///
/// Le corpus source est constitué de collections Firestore **flat top-level**
/// de documents écrits en **camelCase**, dates `int` millis, statuts à **6
/// valeurs**, champs de contrôle inline (`audioText`/`audioTextHash`), **aucune**
/// métadonnée de sync `updated_at`/`is_deleted`. Ce migrateur transforme un tel
/// corpus vers la forme **canonique** (snake_case, enums camelCase à 4 valeurs,
/// `ZSyncMeta` additif hors-entité) **sans perte de champ métier**, de façon
/// **idempotente** (reprise sûre) et **défensive** (jamais de throw, AD-10).
///
/// ## Composition, PAS réécriture
/// Le mapping **par document** (casse camelCase↔snake, valeur `status` 6→4,
/// interop dates `int`→ISO, préservation `_legacy_…`, ajout additif
/// `is_deleted:false`) est déjà porté par [ZStudyLegacyCodec]. Ce
...
///      cf. TRAP ci-dessous) ;
///   2. un **contrôle d'exhaustivité** de préservation exacte des clés métier ;
...
/// codec sous-jacent reste inchangé (son test reste vert).
///
...
///
/// ## Confinement AD-5 (générique par `Map`)
...
/// dépend d'AUCUN package d'entité (`zcrud_document`/`note`/…).
///
/// ## DRY-RUN — write-back DÉFÉRÉ
...
/// RÉELLES legacy → `WriteBatch` ≤ 450/lot → `serverTimestamp()` pour
/// `updated_at` → cutover repo-par-repo) est **hors périmètre de ce fichier** :
/// il n'est PAS implémenté ici (aucune I/O, aucun `FirebaseFirestore`).
library;
...
/// AD-10, jamais avalés en silence), et le **contrôle de préservation**
...
class ZDocumentMigrationOutcome {
  ...
  /// snake_case OU préservé sous `_legacy_<snake>`). Le contrôle de préservation
  /// est complet ssi `coveredBusinessKeys.length == businessKeysIn.length`.
  final Set<String> coveredBusinessKeys;

  /// `true` ssi **aucune** clé métier legacy n'a été silencieusement perdue
  /// (préservation EXACTE, pas simple existence).
...
class ZLegacyMigrationReport {
  ...
  /// write-back de ces documents est hors périmètre de ce migrateur.
  final List<Map<String, dynamic>> canonicalDocuments;

  /// `true` ssi **aucune** clé métier n'a été perdue sur **tout** le corpus.
...
/// (`zcrud_firestore`). Sans état (`const`-constructible) : opère
/// uniquement sur `Map<String, dynamic>`.
///
/// Compose [ZStudyLegacyCodec] (config legacy par défaut : mapping de valeur
/// `status` 6→4 + préservation `_legacy_status`) — il ne réimplémente NI
/// `camelToSnake` NI le mapping de statut.
class ZLegacyStudyMigrator {
  /// Construit un migrateur.
  ///
  /// [codec] : brique par-document injectable (défaut : config legacy
  ...
  /// Configuration legacy par défaut — mapping de valeur
  ...
  /// Les 6 valeurs de statut LEGACY connues (`FolderDocumentStatus`). Toute
  ...
  /// - Sinon **compose** [ZStudyLegacyCodec.toCanonical] et calcule le contrôle
  ///   de préservation + la trace des défauts défensifs.
  ZDocumentMigrationOutcome migrateDocument(Map<String, dynamic> legacy) {
    // Contrôle calculé sur l'entrée ORIGINALE (jamais mutée) : toute perte dans
    // le pipeline (drop de clé) reste détectable.
    final businessIn = _businessKeys(legacy);

    // Copie défensive : DRY-RUN, l'entrée n'est jamais mutée.
  ...
  /// **DRY-RUN** : aucune I/O, aucune mutation d'entrée, aucun write-back
  /// (hors périmètre de ce migrateur). **Ne throw JAMAIS** (AD-10).
  ...
  /// La détection est RÉCURSIVE et tient compte des alias de clés de sync.
  ///
  /// Deux pièges à éviter, tous deux à perte SILENCIEUSE :
  /// 1. n'inspecter que `doc.keys` déclarerait canonique un document au premier
  ///    niveau propre mais au **contenu imbriqué encore legacy** (ex. les
  ///    `nodes[].edgeColor` récursifs d'un mindmap). Il traverserait inchangé, et
  ...
  ///    ne déclencherait rien : un corpus partiellement migré serait sauté.
  ///
  ...
  bool _isAlreadyCanonical(Map<String, dynamic> doc) {
    ...
    // Même piège que `deleted` : une clé source de renommage
    ...
    // La DÉTECTION doit refléter la CONVERSION : un sous-arbre
    ...
    // tierce (qui conserve PAR CONCEPTION ses clés camelCase) serait classé
    // « non canonique » À JAMAIS, donc re-migré à chaque passage — et ses
    // `valueMappers` réappliqués, rétrogradant `status` de `ready` à
    // `uploading` : exactement le TRAP que la garde d'idempotence doit
    // franchir. `opaqueKeys` la neutralise.
    final opaque = _codec.opaqueKeys;
    // Le principe « la détection doit refléter la conversion » ne suffit pas à
    // moitié : `opaqueKeys` seul ne protège pas assez si la détection descend
    ...
    //
    // Sans la garde qui suit (config `valueMappers` + document à contenu
    // imbriqué camelCase, SANS aucun `opaqueKeys`), le document serait déclaré
    // non canonique à jamais, re-migré à chaque passage, et son `status`
    // rétrogradé `ready` → `uploading`.
    //
  ...
  /// pas des clés métier d'entrée (le contrôle de préservation EXCLUT le
  /// hors-corps).
  ...
  /// Contrôle de préservation : pour chaque clé métier [businessIn], vérifie
  /// qu'elle est **retrouvable** dans [canonical] — soit renommée snake_case,
  /// soit préservée sous `_legacy_<snake>`. Retourne le sous-ensemble **couvert**.
  ///
  /// Une clé **aliasée** (renommage sémantique, ex. `quality` →
  /// `last_quality`) est créditée sur sa clé CIBLE. Sans cela, le contrôle
