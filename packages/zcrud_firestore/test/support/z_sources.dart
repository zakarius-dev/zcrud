// Primitives PARTAGÉES des gardes qui LISENT LES SOURCES de `zcrud_firestore`.
//
...
// plusieurs gardes de ce paquet appliquaient `src.contains(...)`/une regex
// SANS AUCUN dé-commentage, ou dé-commentaient uniquement les lignes
// commençant par `//` (aveugles à un commentaire de BLOC ou de FIN de ligne).
// Une (`z_study_migrator_isolation_test.dart`) dé-commentait le BLOC par une
// regex `/\*[\s\S]*?\*/` appliquée AVANT le retrait des lignes `//` : un `/*`
// cité en PROSE dans un commentaire `//` (ex. pour expliquer ce dé-
// commentateur) aurait fait avaler tout le code jusqu'au PROCHAIN `*/` réel du
// fichier. Ce fichier centralise le dé-commentateur ROBUSTE (patron chat, scan
// caractère par caractère, ordre `//` avant `/*`, littéraux de chaîne
// sautés) pour que la prochaine garde s'appuie dessus.
//
...
/// Racine du package `zcrud_firestore`, que le test tourne depuis
/// `packages/zcrud_firestore` (`flutter test`, convention imposée par
/// CLAUDE.md) ou — en secours — depuis la racine du dépôt.
Directory packageRoot() {
  for (final String p in <String>['.', 'packages/zcrud_firestore']) {
  ...
  fail(
    'racine de zcrud_firestore introuvable depuis ${Directory.current.path}',
  );
}

/// Racine du monorepo (porte `melos.yaml`), cwd-robuste.
Directory repoRoot() {
...
/// `lib/` de `zcrud_core`, cwd-robuste — utilisé par les gardes AD-5 qui
/// vérifient que le domaine reste backend-agnostique.
Directory coreDir() => Directory('${repoRoot().path}/packages/zcrud_core');
...
///
/// 🔴 **L'ordre compte** : le commentaire de LIGNE est reconnu AVANT `/*` — cf.
/// dartdoc de librairie ci-dessus pour le bug concret que cet ordre évite.
...
/// Source d'un fichier, dé-commentée et RECOLLÉE en une seule chaîne.
