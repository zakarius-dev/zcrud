...
/// **SEULE exception `reflectable` autorisée par l'invariant AD-3**.
///
/// DODLP repose historiquement sur `reflectable`/GetX pour la sérialisation
/// de ses modèles ; zcrud ne doit ni imposer `reflectable` à ses autres
/// consommateurs, ni forcer DODLP à en changer. Ce fichier est le **chemin
/// EXACT allowlisté** par `scripts/ci/gate_reflectable.dart`
...
/// jamais au cœur (invariant AD-3), et n'est pas un `zcrud_*` (invariant AD-1 :
/// cœur OUT=0 inchangé).
///
/// **Réflexion INJECTÉE (seam invariant AD-6)** : la capacité d'introspection
/// est un port fin [ZReflectionCapability], PAS un `Get.find` en dur ni un
/// reflector figé. La logique d'adaptation ([ReflectableCodec]) est donc
/// **testable sans** exécuter `initializeReflectable()` — on injecte un
/// double en test. Le câblage sur le VRAI reflector DODLP,
/// `initializeReflectable()` et l'initialisation Firebase restent la
/// responsabilité de l'application hôte (hors `packages/`), ce qui évite
...
/// Port fin de **capacité de réflexion** d'un modèle [T] (seam invariant
/// AD-6).
///
...
/// reflector (branché par l'application hôte) ; en test, par un simple
/// double (aucune dépendance reflectable requise côté test → gate vert).
...
/// [ZReflectionCapability] **injectée** (seam invariant AD-6). C'est un
/// [ZModelAdapter] : `registerInto(registry)` (hérité) rend le modèle
/// décodable/encodable via `registry.decode/encode(kind, …)`, et
/// [fromMapSafe] (hérité) fournit le mode défensif invariant AD-10 — sans que
/// le cœur ne connaisse `reflectable`.
class ReflectableCodec<T extends Object> extends ZModelAdapter<T> {
  ...
  /// [fieldSpecs] éventuels (FOURNIS, pas inférés). Formelle initialisante
  /// privée : l'argument externe reste nommé `capability`.
  ReflectableCodec({
...
/// (branchée par l'application hôte sur le reflector DODLP +
...
/// exercée par les tests de ce paquet** (qui injectent un double) : l'exercer
/// exigerait un `*.reflectable.dart` généré sous `packages/`, que le gate
/// rejetterait. La reconstruction ([fromMap]) délègue à la factory du modèle
/// ([construct]) — patron usuel de fabrique `.fromMap`, dont l'application
/// hôte finalise le câblage.
