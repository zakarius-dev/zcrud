/// Point de composition unique du binding `zcrud_get`.
///
/// Ce paquet fait du **binding** le point d'enrôlement UNIQUE des widgets
/// d'édition servis par les satellites (markdown, intl, geo).
/// [registerZcrudFormFields] **appelle** les registrars/builders exposés par
/// chaque satellite sur un [ZWidgetRegistry] **fourni par l'appelant** — il
/// ne construit JAMAIS de registre en singleton statique interne (invariant
/// AD-4) : l'app possède le registre (via son bootstrap / `ZcrudGetScope`) et
/// l'injecte ensuite dans l'arbre via `ZcrudScope.widgetRegistry`, où le
/// dispatcher `ZFieldWidget` le résout.
///
/// **Anti-pattern écarté** : aucun satellite n'édite CE fichier. Chaque
...
///
/// **Exclusivité html ⇄ markdown** : la voie markdown
...
/// `ZWidgetRegistry` (association `kind → ZFieldWidgetBuilder`). D'autres
/// seams de composition sont des **valeurs de `ZcrudScope`** injectées **à
/// côté** par l'app : `ZcrudScope.selectPresenter` (présentateur de sélection,
/// PAS un registrar de widget), `ZcrudScope.filePicker` (média),
/// `ZcrudScope.colorPicker` (roue HSV côté binding). Le composeur ne les
...
///
/// **Invariant AD-12 (secrets)** : aucune clé Maps n'est embarquée ici ;
...
/// Enrôle en UN point les widgets d'édition des satellites (markdown, intl,
/// geo) dans le [registry] **injecté** (invariant AD-4).
///
...
///   → repli coordonnées-seules, aucun SDK carte, aucun secret — invariant
///   AD-12) ;
...
void registerZcrudFormFields(
  ...
  // Voie rich-text PAR DÉFAUT (markdown/inlineMarkdown/richText — invariant AD-7).
  registerZMarkdownFields(registry, codec: richTextCodec);

  // intl : phone/country enrôlés par `.builder()` (les satellites n'exposent
  // PAS de `registerZ<Pkg>Fields` pour ces kinds — écart connu, conforme :
  ...
  // pris, ou double html) throw de façon déterministe.
