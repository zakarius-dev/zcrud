...
/// Markdown — pour `ZAdaptivePresenter`.
///
/// ## Le besoin, mesuré
///
...
///
/// IFFD n'a pas trouvé le défaut en lisant du code : il l'a trouvé par une
/// **injection restée verte** (une garde exerçait `barrierDismissible: false`
/// sur une feuille, qui ne le lit pas ; le `sha256` prouvait que l'injection
/// avait bien eu lieu). Un paramètre qui ne peut produire aucun effet ne se voit
/// pas dans du code qui compile — il se voit **quand on essaie de s'en servir**.
///
/// ## 🔴 Pourquoi ce fichier vit dans `test/`, et pas dans `lib/`
///
/// Même raison que le catalogue de types de champ
/// (`packages/zcrud_core/test/support/z_field_type_catalog.dart`) :
///
/// * **coût runtime nul** — rien ici n'est exporté par
///   `lib/zcrud_navigation.dart` ;
/// * **FR-26 / NFR-S7 par CONSTRUCTION** — ce fichier porte de la prose
///   destinée aux **développeurs** (en-têtes de colonnes, notes). Hors de
///   `lib/`, elle n'est pas « interdite par convention » : elle est
///   **inatteignable** depuis une application, donc elle ne peut pas s'afficher
///   dans une UI sans localisation. Le document `.md` produit n'est, lui, jamais
///   compilé.
///
/// ## Les propriétés, et où elles sont tenues
///
/// 1. **Exhaustivité — deux maillons.**
///    * *mode* : [kZPresenterModes] est `ZEditionPresentation.values` ; une
///      valeur ajoutée à l'enum crée automatiquement sa colonne.
///    * *paramètre* : [ZPresenterParam] est parcouru par `values`, et
///      [zProbeArgs] en fait un `switch` **expression exhaustif sans
///      `default`** ⇒ ajouter une valeur à l'enum **casse la compilation**.
///      Le lien entre l'enum et la **signature réelle du port** est tenu par une
///      garde qui **parse les deux fichiers du port sur disque**
///      ([zPortParameterNames]) et exige l'égalité des ensembles : un paramètre
///      ajouté à `ZFormPresenter.present` ou à
///      `ZImplicitDismissControl.presentWithDismissControl` sans entrée ici fait
///      **rougir** la garde. 🔴 **Limite nommée** : ce maillon-là est une garde,
///      pas la compilation — c'est le seul de la chaîne qui ne soit pas
///      structurel.
///
/// 2. **Statut MESURÉ, pas déclaré — il n'existe nulle part où en écrire un.**
///    Il n'y a **aucun champ « honoré »** dans ce fichier. Le statut d'une
///    cellule est le résultat d'une **mesure différentielle** : la même surface
///    est présentée deux fois, une fois avec la valeur par **défaut du port**,
///    une fois avec une valeur **contraire** ; on relève à chaque fois la même
///    [ZSurfaceObservation] (contraintes reçues par le contenu, encart de
///    sécurité reçu, rectangle rendu, propriétés du `BottomSheet`, effet d'un
///    tap sur la barrière) et
///    **`honoré ⇔ les deux empreintes DIFFÈRENT`**.
///    Déclarer « honoré » un paramètre que la branche ne lit pas est donc
///    **inexprimable** : il n'y a pas de déclaration.
///
///    🔴 Ce n'est pas tautologique : on ne compare **jamais** la valeur rendue à
///    la constante qui l'a produite (le piège de la garde qui vérifie
///    `maxWidth == 160` après avoir passé `160`). On compare **deux rendus entre
///    eux**. Et l'empreinte relevée est **la même pour tous les paramètres** :
///    aucune observation n'est taillée pour faire réussir sa cellule.
///
/// 3. **Anti-vacuité du couple de valeurs.** Une cellule « inerte » ne vaut que
///    si le couple de valeurs sondé était **capable** de produire une
///    différence. La garde l'exige : **aucun paramètre n'est inerte sur les
///    trois modes** — chacun est mesuré effectif quelque part, ce qui prouve son
///    couple discriminant. Sans cela, « inerte partout » serait indiscernable
///    d'une sonde cassée.
///
/// 4. **Synchronisation prouvée.** [renderZPresenterMatrixMarkdown] rend le
///    document à partir des **mesures**, et la garde compare au fichier **tel
///    qu'il est sur disque**, sans jamais le régénérer avant de comparer.
///
/// ## Portée — ce que cette matrice NE couvre PAS
///
/// Elle couvre **`ZAdaptivePresenter`**, et lui seul. `ZGetFormPresenter`
/// (`zcrud_get`) a sa propre matrice, mesurée par sa propre garde
/// (`packages/zcrud_get/doc/parameter-matrix-z-get-form-presenter.md`) : les
/// deux paquets ne peuvent pas partager ce fichier (le `test/` d'un paquet n'est
/// pas importable, et AD-1 interdit à `zcrud_navigation` de connaître
/// `zcrud_get`). La non-divergence des deux listes de paramètres est tenue
/// autrement : **les deux gardes dérivent la liste du MÊME port sur disque.**
///
/// Recensement des implémentations du port (grep, 2026-08-09) : deux en `lib/`
/// dans tout le dépôt — `ZAdaptivePresenter` et `ZGetFormPresenter`. Aucune dans
/// `zcrud_riverpod` ni `zcrud_provider` (grep négatif `rc=1`). Une
/// implémentation **tierce** n'est tenue par aucune de ces deux gardes.
///
/// ## Totalité (AD-10)
///
/// Aucune fonction de ce fichier ne lève : [zProbeArgs] est totale par
/// exhaustivité, [renderZPresenterMatrixMarkdown] est une pure concaténation, et
/// [zPortParameterNames] retourne un ensemble vide plutôt que d'échouer si un
/// fichier de port est introuvable (c'est la garde qui décide, pas ce fichier).
library;
...
const String kZAdaptiveMatrixDocPath =
...
Future<ZSurfaceObservation> zObserveSurface(
  ...
  await tester.pumpWidget(
    MaterialApp(
      ...
      // réutilisent le même `Element` de `MaterialApp`, donc le même
      // `NavigatorState` — et la route de la sonde précédente reste empilée.
