/// Présentateur **par défaut pur-Flutter** — exécute le mode
/// [ZEditionPresentation] en **Flutter vanilla**.
///
/// [ZAdaptivePresenter] est la réécriture **neutre** (sans gestionnaire d'état
/// ni routeur) d'une présentation GetX ad hoc : les trois branches
/// `Get.to`/`Get.dialog`/`Get.bottomSheet` deviennent
/// `Navigator.push(MaterialPageRoute(...))` / `showDialog` /
/// `showModalBottomSheet` **natifs**, `Get.width`/`Get.height` deviennent
/// `MediaQuery.sizeOf(context)`, et les booléens `fullscreenDialog`/`dialog`
/// deviennent l'`enum` [ZEditionPresentation].
///
/// **Aucun manager (invariants AD-2/AD-15)** : ce fichier n'importe **NI**
...
/// sur la source plutôt que de la recopier localement (dialog ~ largeur
/// bornée, sheet ~ 90 % hauteur, dérivées de `MediaQuery.sizeOf` quand
/// `maxWidth`/`maxHeight` ne sont pas fournis).
...
class ZAdaptivePresenter implements ZFormPresenter, ZImplicitDismissControl {
  ...
  Future<T?> presentWithDismissControl<T>(
    ...
    // Switch EXHAUSTIF sur les 3 valeurs de l'enum ⇒ jamais de `throw`
    // (invariant AD-10).
    switch (mode) {
      case ZEditionPresentation.page:
        ...
        // ni avec `false`), `barrierDismissible` et
      ...
      case ZEditionPresentation.sheet:
        ...
        // ── Feuille CONTRAINTE et ENCADRÉE ───────────────────────────────────
        //
        // Changement VISIBLE pour tout hôte passif, assumé : le défaut du
        // socle est une feuille plus étroite que l'écran, avec un
        ...
        // était déjà la surcharge la plus haute.
        ...
        return showModalBottomSheet<T>(
          ...
          // d'abandon est armé, la voie est désactivée ; sinon `null` ⇒
          // défaut SDK, arbre INCHANGÉ (`true` est le défaut du SDK ⇒ voie
          // historique inchangée).
          enableDrag: allowImplicitDismiss,
          // Voie ORTHOGONALE au glissement : `isDismissible` alimente
          // `ModalBottomSheetRoute.barrierDismissible` (SDK). `false` ⇒ le
          // tap sur la barrière ne déclenche même plus `maybePop`, donc le
          // garde d'abandon n'est PAS consulté — c'est « interdire », là où
          // `allowImplicitDismiss: false` dit « garder ». `true` (défaut SDK
          // et défaut du port) ⇒ arbre et comportement INCHANGÉS pour tout
          // hôte passif.
          ...
          // telle quelle (invariant AD-4 : `null` ⇒ absent de l'arbre).
