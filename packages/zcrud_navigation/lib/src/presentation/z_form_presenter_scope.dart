/// Seam de résolution du **présentateur effectif** (invariant AD-6) —
...
/// ZAdaptivePresenter()` (jamais de `throw` — invariants AD-6/AD-10). Un
/// **binding** surcharge le présentateur en enveloppant l'app :
/// `ZFormPresenterScope(presenter: ZGetFormPresenter(), child: ...)`.
///
/// **Seam LOCAL, `ZcrudScope` intact (invariant AD-1)** : on **NE modifie PAS**
/// le `ZcrudScope` de `zcrud_core` (il n'a aucun slot présentateur et vit dans
/// le cœur — y ajouter un [ZFormPresenter] défini ici forcerait
...
class ZFormPresenterScope extends InheritedWidget {
  ...
  /// présent dans l'arbre. `const` — défaut sûr partagé (invariants AD-6/AD-10).
  ...
  /// **Ne lève jamais** (défaut sûr — invariants AD-6/AD-10).
