/// Politique de **présentation d'édition** — le « maillon
/// manquant » : dérive PUREMENT un [ZEditionPresentation] d'un `ZWindowSizeClass`.
///
/// Une intégration ad hoc typique ne **dérive** pas le mode de présentation du
/// breakpoint : elle le fige au call-site (couplage à une largeur globale).
/// [ZPresentationPolicy] pose la règle `breakpoint → mode`, **surchargeable
/// par app sans modifier le package** (invariant AD-6), **non-`sealed`**
/// (invariant AD-4).
///
/// **Pureté (invariants AD-5/AD-14)** : ce fichier n'importe **que**
/// `zcrud_responsive` (pour l'enum `ZWindowSizeClass`, l'ENTRÉE de [resolve]) —
/// **aucun** `import 'package:flutter/...'`, **aucun** `BuildContext`. La
/// liaison `context → largeur → ZWindowSizeClass` est faite **en amont** par
/// l'appelant (via `ZWindowSizeClass.of(context)` — le présentateur),
...
///
/// **Injectable / surchargeable (invariants AD-6/AD-4)** — trois voies, sans
/// jamais figer une constante ni une fonction top-level non substituable :
...
class ZPresentationPolicy {
  ...
  /// règle **sans** sous-classer (invariant AD-6). Ex.
  /// `ZPresentationPolicy.from((c, {formWeight = ZFormWeight.light}) => ...)`.
  ...
  /// **jamais de `throw`** — invariant AD-10). `formWeight` par défaut =
  /// [ZFormWeight.light].
  ///
