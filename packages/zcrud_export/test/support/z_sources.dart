// Squelette PARTAGÉ des gardes qui LISENT LES SOURCES de `zcrud_export`.
//
// 🔴 P0D3 (chantier dartdoc) : `isolation_gates_test.dart` dépouillait déjà
// les commentaires (`_code()`), mais avec un dé-commentateur REGEX « bloc
// PUIS ligne » appliqué sur le fichier ENTIER
// (`replaceAll(RegExp(r'/\*.*?\*/', dotAll))` avant le split par ligne). Cet
// ordre est FRAGILE : un `/*` apparu DANS un commentaire `//`/`///` (une
// dartdoc citant un chemin de ce genre) peut être pris pour l'ouverture d'un
// vrai bloc et AVALER tout le reste du fichier jusqu'au prochain `*/` réel —
// exactement le risque que documente
...
//     exacts).
//
// Ce fichier n'est pas un `*_test.dart` lui-même.
library;
