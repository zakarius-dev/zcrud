...
part of 'z_document_reading_state.dart';
...
ZDocumentReadingState _$ZDocumentReadingStateFromMap(
  Map<String, dynamic> map,
) => ZDocumentReadingState(
  docId: map['doc_id'] is String ? map['doc_id'] as String : '',
  currentPage: _$asInt(map['current_page']) ?? 1,
  pageCount: _$asInt(map['page_count']),
  prefs:
      _$decodeModel(map['prefs'], ZDocumentViewerPrefs.fromMap) ??
      ZDocumentViewerPrefs.fromMap(const <String, dynamic>{}),
);

extension ZDocumentReadingStateZcrud on ZDocumentReadingState {
...
/// Clés que `ZDocumentReadingState.toMap()` PEUT produire — surensemble
...
void registerZDocumentReadingState(ZcrudRegistry registry) {
  ...
  _$zRequireExtraPreserved<ZDocumentReadingState>(
    'ZDocumentReadingState',
    ZDocumentReadingState.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZDocumentReadingState>(
...
const Set<String> $ZDocumentReadingStateTimestampFields = <String>{};
