...
part of 'z_annotation_bounds.dart';
...
ZAnnotationBounds _$ZAnnotationBoundsFromMap(Map<String, dynamic> map) =>
    ZAnnotationBounds(
      x: _$asDouble(map['x']) ?? 0.0,
      y: _$asDouble(map['y']) ?? 0.0,
      width: _$asDouble(map['width']) ?? 0.0,
      height: _$asDouble(map['height']) ?? 0.0,
    );

extension ZAnnotationBoundsZcrud on ZAnnotationBounds {
...
const List<ZFieldSpec> $ZAnnotationBoundsFieldSpecs = <ZFieldSpec>[
  ...
  ZFieldSpec(name: 'y', type: EditionFieldType.float),
  ZFieldSpec(name: 'width', type: EditionFieldType.float),
  ZFieldSpec(name: 'height', type: EditionFieldType.float),
];

/// Clés que `ZAnnotationBounds.toMap()` PEUT produire — surensemble
...
void registerZAnnotationBounds(ZcrudRegistry registry) =>
    registry.register<ZAnnotationBounds>(
      'annotation_bounds',
      fromMap: ZAnnotationBounds.fromMap,
      toMap: (value) => value.toMap(),
      fieldSpecs: $ZAnnotationBoundsFieldSpecs,
    );
...
const Set<String> $ZAnnotationBoundsTimestampFields = <String>{};
