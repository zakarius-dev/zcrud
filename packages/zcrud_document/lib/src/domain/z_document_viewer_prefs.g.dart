...
part of 'z_document_viewer_prefs.dart';
...
ZDocumentViewerPrefs _$ZDocumentViewerPrefsFromMap(Map<String, dynamic> map) =>
    ZDocumentViewerPrefs(
      zoomLevel: _$asDouble(map['zoom_level']) ?? 1.0,
      scrollDirection:
          _$enumFromName(
            ZDocumentScrollDirection.values,
            map['scroll_direction'],
          ) ??
          ZDocumentScrollDirection.values.first,
      pageLayout:
          _$enumFromName(ZDocumentPageLayout.values, map['page_layout']) ??
          ZDocumentPageLayout.values.first,
    );

extension ZDocumentViewerPrefsZcrud on ZDocumentViewerPrefs {
...
const List<ZFieldSpec> $ZDocumentViewerPrefsFieldSpecs = <ZFieldSpec>[
  ...
  ZFieldSpec(name: 'scroll_direction', type: EditionFieldType.select),
  ZFieldSpec(name: 'page_layout', type: EditionFieldType.select),
];

/// Clés que `ZDocumentViewerPrefs.toMap()` PEUT produire — surensemble
...
void registerZDocumentViewerPrefs(ZcrudRegistry registry) =>
    registry.register<ZDocumentViewerPrefs>(
      ...
      fromMap: ZDocumentViewerPrefs.fromMap,
      toMap: (value) => value.toMap(),
      fieldSpecs: $ZDocumentViewerPrefsFieldSpecs,
    );
...
const Set<String> $ZDocumentViewerPrefsTimestampFields = <String>{};
