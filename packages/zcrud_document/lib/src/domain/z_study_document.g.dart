...
ZStudyDocument _$ZStudyDocumentFromMap(Map<String, dynamic> map) =>
    ZStudyDocument(
      ...
      status:
          _$enumFromName(ZDocumentStatus.values, map['status']) ??
          ZDocumentStatus.values.first,
      storagePath: map['storage_path'] is String
          ? map['storage_path'] as String
          : '',
      pageCount: _$asInt(map['page_count']),
      sizeBytes: _$asInt(map['size_bytes']) ?? 0,
      createdAt: _$asDateTime(map['created_at']),
    );

extension ZStudyDocumentZcrud on ZStudyDocument {
  ...
  ZStudyDocument copyWith({
    ...
    folderId: identical(folderId, _$undefined)
        ? this.folderId
        : folderId as String,
    fileName: identical(fileName, _$undefined)
        ? this.fileName
        : fileName as String,
    ...
    pageCount: identical(pageCount, _$undefined)
        ? this.pageCount
        : pageCount as int?,
    sizeBytes: identical(sizeBytes, _$undefined)
        ? this.sizeBytes
        : sizeBytes as int,
...
/// `toMap()`/`copyWith()` de `ZStudyDocument` en MEMBRES D'INSTANCE.
///
/// À appliquer (`class ZStudyDocument … with _$ZStudyDocumentZcrud`) quand un membre
...
/// de base. Corps identiques à ceux de l'extension `ZStudyDocumentZcrud` : la map
/// produite ne change pas. Les champs déclarés par la classe deviennent alors
/// des `@override` des getters ci-dessous.
mixin _$ZStudyDocumentZcrud {
  String? get id;
  String get folderId;
  String get fileName;
  ZDocumentStatus get status;
...
const List<ZFieldSpec> $ZStudyDocumentFieldSpecs = <ZFieldSpec>[
  ...
  ZFieldSpec(name: 'storage_path', type: EditionFieldType.text),
  ZFieldSpec(name: 'page_count', type: EditionFieldType.integer),
  ZFieldSpec(name: 'size_bytes', type: EditionFieldType.integer),
  ZFieldSpec(name: 'created_at', type: EditionFieldType.dateTime),
];

/// Clés que `ZStudyDocument.toMap()` PEUT produire — surensemble
...
void registerZStudyDocument(ZcrudRegistry registry) {
  ...
  _$zRequireExtraPreserved<ZStudyDocument>(
    'ZStudyDocument',
    ZStudyDocument.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZStudyDocument>(
    'study_document',
...
const Set<String> $ZStudyDocumentTimestampFields = <String>{};
