...
part of 'z_document_annotation.dart';
...
ZDocumentAnnotation _$ZDocumentAnnotationFromMap(Map<String, dynamic> map) =>
    ZDocumentAnnotation(
      id: map['id'] is String ? map['id'] as String : null,
      docId: map['doc_id'] is String ? map['doc_id'] as String : '',
      page: _$asInt(map['page']) ?? 1,
      kind:
          _$enumFromName(ZDocumentAnnotationKind.values, map['kind']) ??
          ZDocumentAnnotationKind.values.first,
      colorKey: map['color_key'] is String ? map['color_key'] as String : '',
      bounds:
          _$decodeModel(map['bounds'], ZAnnotationBounds.fromMap) ??
          ZAnnotationBounds.fromMap(const <String, dynamic>{}),
      rects: map['rects'] is List
          ? (map['rects'] as List)
                .map((e) => _$decodeModel(e, ZAnnotationBounds.fromMap))
                .whereType<ZAnnotationBounds>()
                .toList()
          : null,
...
extension ZDocumentAnnotationZcrud on ZDocumentAnnotation {
...
const List<ZFieldSpec> $ZDocumentAnnotationFieldSpecs = <ZFieldSpec>[
  ...
  ZFieldSpec(name: 'page', type: EditionFieldType.integer, defaultValue: 1),
  ZFieldSpec(name: 'kind', type: EditionFieldType.select),
  ZFieldSpec(name: 'color_key', type: EditionFieldType.text),
  ZFieldSpec(name: 'bounds', type: EditionFieldType.subItems),
  ZFieldSpec(name: 'rects', type: EditionFieldType.subItems, multiple: true),
  ZFieldSpec(name: 'text', type: EditionFieldType.text),
  ZFieldSpec(name: 'created_at', type: EditionFieldType.dateTime),
];

/// Clés que `ZDocumentAnnotation.toMap()` PEUT produire — surensemble
...
void registerZDocumentAnnotation(ZcrudRegistry registry) {
  ...
  _$zRequireExtraPreserved<ZDocumentAnnotation>(
    'ZDocumentAnnotation',
    ZDocumentAnnotation.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZDocumentAnnotation>(
    'document_annotation',
...
const Set<String> $ZDocumentAnnotationTimestampFields = <String>{};
