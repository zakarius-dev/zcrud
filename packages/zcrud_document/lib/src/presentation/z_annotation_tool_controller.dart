/// `ZAnnotationToolController` — état local et isolé de la toolbar
/// d'annotation.
///
/// Réactivité Flutter-native, aucun gestionnaire d'état (invariants AD-2/
/// AD-15) : `ChangeNotifier` pur-Flutter (`package:flutter/foundation.dart`
/// uniquement) — jamais un gestionnaire d'état tiers, jamais un `WidgetRef`/
/// équivalent. L'état mutable de la toolbar (le `kind` sélectionné et la
/// `colorKey` sélectionnée) est exposé une tranche à la fois via une
/// [ValueListenable] par champ : un widget qui n'écoute que
/// [selectedColorKey] ne se reconstruit pas quand [selectedKind] change, et
/// inversement (rebuild ciblé, zéro reconstruction globale).
///
/// Patron owned/injected : la toolbar crée un controller en `initState` ssi
/// aucun n'est injecté et le `dispose` ssi elle le possède — jamais recréé
/// au `build`.
///
/// `colorKey` reste une `String` opaque (invariant AD-4) : le controller ne
/// connaît aucune `Color`, aucune palette, aucun index — la résolution
/// `colorKey → Color` est un seam de présentation
...
/// Préfixe de [ValueKey] d'un bouton de `kind` (`zAnnotationKind_<name>`).
const String kAnnotationKindKeyPrefix = 'zAnnotationKind_';

/// Préfixe de [ValueKey] d'une swatch de `colorKey`
/// (`zAnnotationSwatch_<key>`).
...
/// réellement rendue (contraste mesuré, couleur injectée).
...
/// « coché ») — présent uniquement dans la swatch sélectionnée (la
...
/// (`zAnnotationPanelEntry_<id|index>`).
...
/// notifie que ses écouteurs (rebuild ciblé).
class ZAnnotationToolController extends ChangeNotifier {
  /// Construit le controller sur un état initial (défauts défensifs alignés
  /// sur le domaine : `kind` = première constante déclarée, `colorKey`
  /// brute vide).
  ...
  /// Tranche « `colorKey` sélectionnée » (`String` opaque, invariant AD-4 ;
  /// écoute ciblée).
  ...
  /// [selectedColorKey]. Aucune normalisation de palette ici.
