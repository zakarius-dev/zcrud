/// Ergonomie typée `ZFlashcardType` sur `ZStudySessionConfig`, restituée côté
/// `zcrud_flashcard`.
///
/// Le noyau `zcrud_study_kernel` neutralise `ZStudySessionConfig.types` en
/// `List<String>?` (clés opaques camelCase) pour préserver l'acyclicité du
/// graphe de dépendances (invariant AD-1) et éviter que `ZFlashcardType`
/// n'entre dans le noyau. Cette extension rend l'API typée aux consommateurs
/// flashcard sans modifier le format persisté ni le noyau :
/// - [flashcardTypes] : lit `types` et le mappe vers `List<ZFlashcardType>`
///   en ignorant défensivement toute clé inconnue (invariant AD-10) ;
...
/// Adaptateur typé `ZFlashcardType` pour `ZStudySessionConfig`.
extension ZStudySessionConfigFlashcardX on ZStudySessionConfig {
  /// Types filtrants typés : `types` mappé en `ZFlashcardType`, clés
  /// inconnues ignorées (drop défensif, invariant AD-10). Retourne `null` si
  /// `types` est `null` (pas de filtre), sinon une liste (éventuellement
  /// vide).
...
/// Résout un [ZFlashcardType] depuis son `name` camelCase, ou `null` si
/// inconnu.
