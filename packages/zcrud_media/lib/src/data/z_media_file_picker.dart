...
/// Câble `image_picker`/`file_picker`/`image_cropper` derrière une **API
/// neutre** : [pick] retourne des [AppFile] en
/// [ZAppFileUploadState.pending] (`localPath`/`name`/`mimeType`, **jamais**
/// d'octets — contrat cœur, invariant AD-2).
///
/// Aucun type plateforme (`XFile`/`PlatformFile`/`CroppedFile`) n'apparaît en
/// signature ni en valeur de tranche. Dépend du seul `zcrud_core` (+
/// dépendances média confinées, invariant AD-1). **Invariant AD-10** : toute
...
///
/// **Seams injectables** : chaque chemin (galerie/caméra/
...
class ZMediaFilePicker implements ZFilePicker {
  ...
  /// [ZPluginImageCropSeam]). [scanSeam] est **`null` par défaut** (aucune
  /// implémentation de numérisation fournie ici ⇒ `pick(source: scan)`
  /// retourne `[]` défini). [cropOptions] pilote le recadrage post-pick des
  /// images (désactivé par défaut ⇒ flux d'acquisition inchangé).
  ZMediaFilePicker({
    ...
    try {
      switch (source) {
        ...
        case ZFileSource.camera:
          // Capture unique déléguée à l'appareil photo du système d'exploitation.
          final picked = await _imageSeam.pickImages(
            fromCamera: true,
            multiple: false,
            limit: 1,
        ...
        case ZFileSource.scan:
          // Sans seam scan injecté, résultat défini vide (invariant AD-10).
          final scanner = _scanSeam;
          if (scanner == null) return const <AppFile>[];
          return await scanner.scan();
      }
    } catch (_) {
      // invariant AD-10 : rempart ultime — un seam qui throw ne traverse
      // jamais la façade.
  ...
  /// conservé** (invariant AD-10) ; désactivé ⇒ liste inchangée.
