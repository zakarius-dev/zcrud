/// Seams d'acquisition média **injectables**.
///
/// **API neutre** : chaque seam expose des méthodes dont les
...
/// `lib/src/data/` (confinées) ; les tests injectent des **fakes**
/// déterministes plutôt que le vrai plugin, non testable sans appareil.
///
/// **Invariant AD-10** : chaque contrat garantit un **résultat défini** (liste
/// vide / `null` / `false`) sur annulation, permission refusée ou plugin
/// défaillant — **jamais** un throw traversant. La façade [ZMediaFilePicker]
/// renforce cette garantie en enveloppant tout appel de seam.
library;
...
///
/// [fromCamera] `true` ⇒ capture caméra (délégation à l'appareil photo du
/// système d'exploitation) ; sinon galerie. [multiple] autorise la multi-sélection
...
/// conserve l'original (invariant AD-10). Jamais de throw traversant.
...
///
/// Aucune implémentation par défaut n'est fournie ici (dépendance native à
/// risque, hors du périmètre de ce paquet) : ce seam reste donc
/// **injectable** et **`null` par défaut** dans [ZMediaFilePicker] ⇒
/// `pick(source: scan)` retourne un `[]` **défini** (invariant AD-10), jamais
/// un throw. Un hôte qui a besoin de numérisation fournit sa propre impl
/// concrète. Retourne des [AppFile] neutres (idéalement un PDF assemblé).
/// Annulation ⇒ `[]`.
...
/// Seam d'**ouverture** d'un fichier au tap, neutre.
///
...
/// **résultat défini**, jamais un throw traversant (invariant AD-10).
