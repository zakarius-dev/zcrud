...
///
/// Invariants portés : AD-5 (backend-agnostique) ; AD-9 (offline-first,
/// soft-delete hors-entité) ; AD-10 (défensif) ; AD-11 (`Either`/flux nus) ;
/// AD-14 (matérialisation de l'éphémère).
library;
...
/// l'affaire du repository/orchestrateur — **hors** de ce port (aucune
/// méthode `sync()`/`merge()` ici, frontière **volontaire**).
///
...
///
/// **Invariants portés par l'impl** (AD-9/AD-14, documentés ici, non
/// implémentés) :
...
///   AD-9) **sans** toucher aux champs métier ; la suppression locale est un
...
abstract class ZLocalStore<T extends ZEntity> {
  ...
  ///
  /// **ÉCRASEMENT TOTAL** : le document persisté est **remplacé** par la
  ...
  /// Pour préserver l'existant non mappé, voir [putMerged].
  Future<ZResult<T>> put(T item);

  /// **Écriture PRÉSERVANTE** : fusionne la sérialisation de [item]
  ...
  /// - **Ce merge est ADDITIF** : il ne peut pas EFFACER une clé. Un champ
  ...
  /// **Voie de lecture de SYNCHRONISATION** : lit **toutes** les entrées
  ...
  /// Last-Write-Wins, qui doit voir de **chaque côté** l'`updated_at` ET
  ...
  /// **Purge physique par identité** : supprime DÉFINITIVEMENT
  ...
  /// tombstone au distant. Si la suppression doit se **synchroniser**, purger
  /// retire le tombstone qui la portait ⇒ un autre appareil **RESSUSCITE** le
  /// document au prochain `sync()`. **C'est une perte de donnée**, silencieuse.
  ///
  ...
  /// émis.
  ///
  /// **N'utiliser QUE si l'annulation est STRICTEMENT locale** — une écriture
  /// qui n'a jamais atteint le distant. Sinon :
  /// - suppression **utilisateur** à propager → [softDelete] ;
  /// - annulation à propager **sans** garder de tombstone local → une méthode
  ///   dédiée du repository qui propage **puis** purge, dans cet ordre.
  ///
  ...
  /// entité (celle-ci reste un soft-delete propagé au distant).
