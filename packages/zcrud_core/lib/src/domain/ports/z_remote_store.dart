...
/// distant fire-and-forget) ; AD-11 (`Either`/flux nus).
library;
...
/// Last-Write-Wins** sur `updatedAt`, la cascade bornée et le
/// **débounce** (`ZSyncOrchestrator`) ne vivent **pas** ici (frontière
/// **volontaire** — aucune orchestration dans ce port).
///
/// **Backend-agnostique** (AD-5) : aucune signature n'expose de type
/// `cloud_firestore`. L'adaptateur concret (`FirestoreZRemoteStore`,
/// `zcrud_firestore`) délègue au repository Firestore ; les dates
...
abstract class ZRemoteStore<T extends ZEntity> {
  ...
  /// **Écriture PRÉSERVANT la méta** d'une **seule** entrée : écrit
  ...
  /// jamais `now()`).
  ///
  /// La **borne** de découpage (taille sûre sous la limite du backend) est
  /// **backend-spécifique** et vit **exclusivement** dans l'adaptateur
