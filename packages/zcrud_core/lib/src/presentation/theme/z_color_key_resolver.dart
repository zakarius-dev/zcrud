/// Seam de résolution `colorKey (String) → couleur` — **jumeau** du seam
/// d'icône `ZAdornmentIconResolver`/`zResolveAdornmentIcon`
/// (`z_field_adornment_view.dart`).
///
/// ## Le motif copié (et pourquoi)
///
/// Le seam d'icône expose **trois** pièces : le `typedef` (seam hôte), une
...
/// de puce, non).
///
/// ## Zéro table sémantique métier dans le cœur
///
/// `zcrud_core` est le **puits** du graphe (AD-1, out-degree 0) : il **ne peut
/// pas** dépendre d'un satellite. Il ne doit donc **pas** répliquer la
/// liste de clés sémantiques d'un domaine applicatif (`success`/`warning`/`info`/…) :
/// ce serait une duplication **non testable** (aucun paquet ne voit les deux),
/// qui dériverait silencieusement dès qu'un satellite ajouterait une clé.
///
/// Ce fichier ne connaît donc **aucune clé applicative**. Son vocabulaire est celui
/// du `ColorScheme` **Material 3** lui-même, réifié par [ZColorSlot] : le
/// « défaut » n'est pas une table, c'est l'`enum` (`slot.name == colorKey`).
/// La **source unique** des clés d'un domaine applicatif reste sa propre
/// palette, et le pont entre les deux est un **entier** (un index → [ZColorSlot]),
/// pas une liste de `String` dupliquée.
///
/// ## Contraste garanti (AD-13)
///
...
/// Une sémantique success/warning **exacte** relève du
...
/// fonds contrastés (AD-13). C'est le **seul** vocabulaire de couleur du
...
/// typiquement une `colorKey` déjà remappée par la palette applicative) en
...
///
/// La signature **compose** avec celle du repli : elle porte le
...
/// substituable tel quel). Le domaine applicatif ne porte JAMAIS de
/// `Color` (AD-3).
...
/// **n'invente pas** de teinte absente du `ColorScheme`. L'appelant
...
///    la clé dans sa palette applicative — couleurs distinctes,
///    déterministes et contrastées, sans sémantique prétendue.
///
/// Jamais de throw (AD-10). Aucun littéral hexadécimal codé en dur.
