...
/// **sa seule tranche** de `ZFormController` (invariant AD-2).
///
/// Chaque champ est un widget top-level qui n'écoute QUE
/// `fieldListenable(name)` — taper dans un champ ne reconstruit QUE ce champ
/// (jamais un voisin, jamais le formulaire). Corrige par conception le bug
/// historique de rebuild global (jank, perte de focus, saut de curseur).
///
/// INVARIANTS (invariant AD-2, NON-NÉGOCIABLES) :
/// - **Frontière de rebuild = la tranche** : le rendu vit sous
///   [ZFieldListenableBuilder] (helper RÉUTILISÉ, jamais réimplémenté) ;
...
///   oracle du « le champ a-t-il le focus ? » pour la **synchronisation
///   guardée**. Une valeur changée de l'EXTÉRIEUR se reflète dans le
///   `TextField` **quand le champ n'a PAS le focus**, mais n'est **JAMAIS**
///   ré-injectée en écrasant la sélection **quand le champ édite** — priorité
///   absolue à la saisie et au curseur en cours (y compris curseur AU MILIEU).
/// - **Validation ciblée** : rendu par un `TextFormField` **autonome**
///   (sans `Form`/`FormBuilder` global — invariant AD-2) portant
...
///   `key: ValueKey(field.name)` — un rebuild externe réutilise alors
///   l'`Element`/`State` (état de saisie préservé).
///
/// RENDU **type-agnostique** : un `TextFormField` uniforme suffit à prouver
/// la granularité sur N champs. Le **dispatcher par type** (`ZFieldWidget`
/// texte/nombre/date/booléen/select/relation) + a11y/RTL par-widget vivent
/// dans un fichier séparé : ce widget est volontairement neutre pour que le
/// dispatch échange le rendu interne sans toucher ni la machinerie de tranche,
/// ni le contrat de stabilité, ni la compilation de validateurs.
///
/// Aucun gestionnaire d'état (invariant AD-15) : seules les primitives
/// Flutter (`ChangeNotifier`/`ValueListenable`) et `form_builder_validators`
/// (validateurs PURS, jamais un état) sont utilisées.
library;
...
/// stable — invariant AD-2) ; sans quoi un rebuild externe pourrait voler
/// l'état d'un voisin ou recréer le `TextEditingController`.
class ZEditionField extends StatefulWidget {
  ...
  /// Spécification `const` du champ rendu (`name`/`label`/…).
  ...
  /// le `builder` du slice) — compteur de build par champ (invariant AD-2).
  @visibleForTesting
...
class _ZEditionFieldState extends State<ZEditionField> {
  /// `TextEditingController` interne — créé UNE FOIS, jamais recréé
  /// (invariant AD-2). Sa valeur n'est écrite QUE par la **sync guardée** hors
  /// focus (jamais dans la voie de frappe, jamais pendant l'édition).
  late final TextEditingController _text;

  /// `FocusNode` **stable** — créé une fois, `dispose`. Oracle « le champ a le
  /// focus ? » de la sync guardée.
  ...
  Widget build(BuildContext context) => ZFieldListenableBuilder(
        ...
        builder: (context, value, child) {
          ...
          // SYNC GUARDÉE : refléter une valeur EXTERNE dans le champ
