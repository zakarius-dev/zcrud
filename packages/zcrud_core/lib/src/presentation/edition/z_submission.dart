/// Soumission agrégée d'un formulaire d'édition + états UI accessibles
/// (invariants AD-11/AD-2/AD-15).
///
/// Un formulaire se ferme par une **voie de soumission** create/update
/// robuste. [ZEditionSubmitController] valide TOUS les champs visibles (plat
/// + toutes les étapes d'un stepper, conditionnels honorés), puis délègue à
/// un **seam** applicatif `onSubmit` retournant `Future<Either<ZFailure, T>>`
/// (invariant AD-11) — le hook et les valeurs sont détenus **hors** du
/// `ZFormController` (jamais dans une tranche, jamais traversés par le
/// codegen — invariant AD-3).
///
/// INVARIANTS (NON-NÉGOCIABLES) :
/// - **Invariant AD-11** : `onSubmit` retourne `Either<ZFailure, T>`. Le
///   cœur s'arrête à [ZSubmissionState.failure] ; **aucun `AsyncValue`**
///   ici. Le pont `AsyncValue.error` (un provider qui déplie l'`Either` et
///   **re-throw** l'exception typée) vit dans **`zcrud_riverpod`** — jamais
///   importé dans `zcrud_core` (invariant AD-15). Une exception jetée par
///   `onSubmit` est **enveloppée** en [ZFailure] (`ZServerFailure`), jamais
///   un `catch(_){}` nu.
/// - **Invariant AD-2** : l'état de soumission est un `ValueListenable`
///   **dédié** — le bouton et la surface d'erreur n'écoutent QUE lui ; aucun
///   rebuild global sur la voie de frappe. La validation agrégée ne monte
///   AUCUN `Form` global : elle exécute les validateurs mémoïsés (champ-local
///   + inter-champs) contre `controller.valueOf`.
library;
...
/// Statut de la soumission. `failure` couvre l'échec de **validation**
...
/// Type-valeur **immuable** de l'état de soumission.
@immutable
...
class ZSubmissionOutcome<T> {
  ...
  /// Échec de **validation** agrégée : `onSubmit` NON appelé.
  ...
  /// Échec **applicatif** (`Left` ou exception enveloppée — invariant AD-11).
  const ZSubmissionOutcome.failure(ZFailure failure)
      : this._(ZSubmissionStatus.failure, failure: failure);

  /// Soumission **ignorée** (ré-entrance pendant `inProgress`).
  ...
  /// `true` si l'issue est un échec de **validation**.
  bool get isValidationFailure => failure is ZValidationFailure;
...
/// Seam applicatif de soumission (invariant AD-11) : reçoit un **snapshot de
/// données** (`Map<String, Object?>`) et retourne `Future<Either<ZFailure, T>>`.
/// Détenu HORS du `ZFormController` (jamais sérialisé — invariant AD-3).
/// L'implémentation réelle (repository create/update) vit dans l'app hôte ;
/// ce contrôleur se teste avec un `onSubmit` factice.
typedef ZOnSubmit<T> = Future<ZResult<T>> Function(Map<String, Object?> values);

/// Contrôleur de **soumission** consommant un [ZFormController].
///
/// Séparé du `ZFormController` (décision : contrôleur dédié) : le
...
/// [state] (invariant AD-2).
///
/// Pont `AsyncValue.error` (invariants AD-11/AD-15) : ce contrôleur s'arrête
/// à [ZSubmissionState.failure]. Un provider de **`zcrud_riverpod`** déplie
/// l'état (ou l'`Either` retourné par [submit]) et **re-throw** l'exception
/// typée pour alimenter `AsyncValue.error`. Ce pont est DOCUMENTÉ ici mais
/// n'importe RIEN de Riverpod dans le cœur.
class ZEditionSubmitController<T> {
  ...
  /// État de soumission observable. Le bouton/la surface d'erreur
  /// n'écoutent QUE ce `ValueListenable` (invariant AD-2).
  ...
  /// délègue à [onSubmit] avec un snapshot immuable des valeurs.
  ///
  /// - Invalide ⇒ [controller.revealErrors] (révèle TOUTES les familles),
  ...
  /// - Exception jetée par `onSubmit` ⇒ **enveloppée** en `ZServerFailure`
  ///   (invariant AD-11).
  ...
  Future<ZSubmissionOutcome<T>> submit() async {
    ...
    try {
      ...
      // Invariant AD-11 : jamais de remontée non typée — on enveloppe en ZFailure.
