...
### Ajouté
- **`ZReferenceProfile` + jeton `ZcrudTheme.referenceProfile`** : le socle rend désormais par défaut une **apparence de référence** (profil `legacy`, valeur d'un jeton nul). L'échappatoire est **unique et globale** : `ZcrudScope(theme: ZcrudTheme(referenceProfile: ZReferenceProfile.neutral))` ramène le rendu à celui d'avant, strictement — l'inertie est prouvée par égalité de l'arbre de widgets (rectangles au dixième de point, insets, décorations) sur trois largeurs, avec et sans icône.
- **`zLegacyOr` / `zLegacyOrIn`** : arbitrage d'un membre **couleur** entre référence et profil neutre, au dernier maillon de la chaîne **paramètre > jeton > référence**. Les scalaires ne passent jamais par là : ils restent remplaçables jeton par jeton dans les deux profils.
- **`ZSignaturePaletteReference`** (`lib/src/presentation/theme/z_signature_palette_reference.dart`) : **unique** fichier de référence couleur de cette famille, exempté **nominativement par chemin exact** dans la garde de source anti-couleurs (FR-26). Quatre familles : `gradients` (5), `deepGradients` (5), `mutedGradients` (5), `subjectGradients` (8 = les 5 de base + 3). Chaque `onGradient` est **mesuré** par contraste WCAG contre la bande médiane du dégradé (plancher 3.0:1, recalculé par une garde sur les 23 spécifications) — jamais un blanc décrété.
- **Clé de dégradé `zcrud.signature.<identité>`** (`zSignatureKeyPrefix`, `zSignatureKey`) : dernier maillon de `zResolveGradient`, qui répond avec la palette signature sous le profil `legacy`. Les préfixes `zcrud.fieldType.*` et `zcrud.fieldAccent.*` restent **seam-only** — un formulaire sans résolveur d'hôte ne se teinte toujours pas.
- **`ZPaletteIndexStrategy`** (`titleHash`, `ordinal`, `stableFnv`) + `zPaletteIndexFor` + `zSignatureGradientFor` (fonction pure). Défaut `titleHash` par fidélité au rendu de référence ; sa dartdoc avertit que `String.hashCode` **n'est pas stable** entre plateformes ni versions de SDK. `stableFnv` (FNV-1a 32 bits sur les unités UTF-16) est reproductible et couvert par des vecteurs figés.
- **Six jetons nullables** câblés aux quatre sites : `referenceProfile`, `signaturePalette`, `signaturePaletteIndexStrategy`, `sectionHeaderAccentHeight` (défaut 3), `sectionHeaderIconTileSize` (défaut 36), `sectionHeaderIconTileRadius` (défaut 10). Les trois premiers ne s'interpolent pas (bascule à mi-course) ; les scalaires suivent le repli de plancher habituel.

### Modifié
- 🔴 **RUPTURE ASSUMÉE pour tout hôte, y compris passif** — `DynamicEdition` : les en-têtes de section (`_SectionHeader` et `_CollapsibleSectionHeader`, chemin natif comme chemin stylé) gagnent **par défaut** une bande d'accent de 3 dp colorée par le dégradé signature indexé sur le titre, et, quand une icône est déclarée, une **tuile de 36 dp au rayon 10** portant un lavis du même dégradé. La hauteur des en-têtes augmente donc de 3 dp, et de 12 dp de plus pour un en-tête à icône. Un `ZEditionSectionStyle.topAccent` déclaré continue de l'emporter sur la référence. Pour retrouver exactement le rendu antérieur : poser `referenceProfile: ZReferenceProfile.neutral`.
- **Dartdoc de `ZFieldTintPresets`** réécrite en contrat (ce que l'API garantit, comment l'appeler) et distinguée de la palette signature, qui, elle, est lue par défaut.
- 🔴 **`ZcrudScope` est désormais un `InheritedTheme`** : il est **capturé par les routes poussées**, exactement comme un `Theme`. Un dialogue, une feuille modale, un menu ou un sélecteur ouvert par `showDialog` / `showModalBottomSheet` / `showMenu` / `showDatePicker` depuis un descendant du scope voit maintenant ses jetons (`theme`), ses libellés (`labels`), ses seams et son ACL — alors qu'il retombait jusqu'ici sur les défauts du paquet (ACL **refusante**, jetons dérivés du `ColorScheme`), parce qu'une route naît dans l'`Overlay` du `Navigator` et n'héritait que des `ThemeData.extensions`. `updateShouldNotify` est inchangé, et `wrap` re-pose le **même bundle** (mêmes instances de seams, de contrôleurs et de registres) : une re-pose ne notifie aucun dépendant. La règle de sécurité est inchangée : hors de tout scope capturable, l'ACL reste **fail-closed** — la capture propage un scope existant, elle n'en fabrique jamais un.
  ⚠️ Un hôte qui **compensait** ce défaut en re-posant un `ZcrudScope` (ou en transportant ses jetons à la main) à l'intérieur de ses dialogues/feuilles/menus se retrouve avec une re-pose **redondante mais identique** : elle peut être retirée sans danger. Un hôte **passif** ne voit rien changer hors des routes poussées, où le scope devient visible.
- **Sous-liste, forme `page`** : le formulaire d'item, seul présentateur du paquet qui pousse sa route à la main (`Navigator.push(MaterialPageRoute(…))`), capture désormais explicitement la pile de thèmes hérités (`InheritedTheme.capture` + `wrap`). Il relevait jusqu'ici le seul `ZcrudScope` et le re-posait — ce qui laissait tomber tout le reste de la pile (un `Theme`, un `IconTheme`, un `DefaultSelectionStyle` posés entre le `Navigator` et le champ). Cette compensation interne est retirée au profit du mécanisme du framework, qui couvre les deux.

### Gardes
- Table des 18 dégradés de référence **figée dans le test**, relevée `fichier:ligne` sur le code de référence et jamais relue depuis la référence elle-même ; contraste `onGradient` recalculé ; premier plan réellement départagé (ni tout blanc, ni tout noir).
- Inertie absolue sous `neutral` : arbre de `_SectionHeader` et de `_CollapsibleSectionHeader` strictement égal à celui d'avant le lot, sur trois largeurs.
- Exemption nominative de la garde de pureté accompagnée de **trois contre-preuves** : un chemin exempté inexistant rougit, un chemin exempté sans littéral de couleur rougit, et le même contenu placé sous un autre chemin rougit.
- **Routes poussées** (`z_scope_inherited_theme_test.dart`, 10 gardes) : jeton, libellés, seam **à identité égale** et ACL traversent `showDialog`, `showModalBottomSheet` et `showMenu`, le jeton n'étant posé QUE par `ZcrudScope(theme: …)` — aucune `ThemeData.extension`, sans quoi la garde serait verte pour la mauvaise raison. Deux gardes d'**inertie** (sans route poussée, `of` rend la même instance et aucun dépendant ne se reconstruit ; un `wrap` ne notifie pas et ne reprend pas la clé), une garde de **sécurité** (hors scope, la route reste fail-closed et aucun scope n'est inventé) et une garde **adversariale** : avec un scope racine posé au-DESSUS du `Navigator` — donc déjà visible de la route — et un scope dérivé sous lui, la route doit lire le **dérivé le plus proche** ; sans capture elle lit la racine, faux-vert que cette garde interdit.
- **Forme `page` de la sous-liste** (`z_sub_list_page_route_capture_test.dart`) : un `Theme` local posé entre le `Navigator` et le champ doit suivre la route poussée à la main — rouge avant le lot — tandis que l'ACL (à identité égale) et le jeton du scope, que la compensation retirée assurait déjà, restent visibles.
