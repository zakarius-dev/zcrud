...
// remoteUrl) ; échec → Left(ServerFailure) ; delete → Right(unit).
...
class FakeCloudStorageRepository implements CloudStorageRepository {
  ...
  /// Si `true`, [upload] retourne `Left(ServerFailure)`.
  bool fail;
  ...
  Future<ZResult<AppFile>> upload(AppFile file) async {
    uploadCount++;
    if (fail) {
      return Left<ZFailure, AppFile>(const ServerFailure('upload failed'));
    }
