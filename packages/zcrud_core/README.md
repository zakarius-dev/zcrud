...
| `ZRelationCrudHandler` / `ZRelationCrudRegistry` | **CRUD inline** d'une entité liée depuis un sélecteur `relation` (créer/modifier/copier sans quitter le formulaire, option résultante auto-sélectionnée). Les trois gestes se gouvernent **séparément** — `canCreate` / `canEdit` / `canCopy`, à `true` par défaut : modifier sans pouvoir créer s'exprime. Un geste refusé est **absent**, jamais inerte (ni bouton, ni icône, ni action sémantique) ; le socle **ne connaît pas** l'ACL de l'hôte, il lit ce que l'implémentation du port déclare. Lecture par `ZRelationCrudOffer` (`offersCreate`…) : un getter hôte qui lève **ferme** le geste (invariant AD-10). |
| `ZSelectChoiceBuilderRegistry` / `ZSelectChoiceBuilders` | Rendu riche déclaré d'une option `select` : `ZSelectConfig(choiceBuilderKey: '…')` garde la clé `const`; l'hôte injecte les closures `choiceBuilder` et/ou `choiceSecondaryBuilder` par `ZcrudScope.selectChoiceBuilderRegistry`. Le widget de sélection résout lui-même la clé (pas de relais du dispatcher), enfant > parent; clé absente ⇒ rendu par défaut silencieux. En consultation, une clé effectivement résolue conserve ce rendu riche (sous `ZReadModeScope` et avec la spec verrouillée) au lieu de la fiche générique ; sans clé ou sans résolution, la fiche historique reste la règle. |
