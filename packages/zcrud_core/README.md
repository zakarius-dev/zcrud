...
| `ZSelectChoiceBuilderRegistry` / `ZSelectChoiceBuilders` | Rendu riche déclaré d'une option `select` : `ZSelectConfig(choiceBuilderKey: '…')` garde la clé `const`; l'hôte injecte les closures `choiceBuilder` et/ou `choiceSecondaryBuilder` par `ZcrudScope.selectChoiceBuilderRegistry`. Le widget de sélection résout lui-même la clé (pas de relais du dispatcher), enfant > parent; clé absente ⇒ rendu par défaut silencieux. |
