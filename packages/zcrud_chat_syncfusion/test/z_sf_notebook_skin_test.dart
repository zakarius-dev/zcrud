...
Future<SfAIAssistView> _mount(
  ...
  await tester.pumpWidget(
    MaterialApp(home: Scaffold(body: tree)),
  );
  await tester.pump();
...
void _expectSame(AssistMessageSettings a, AssistMessageSettings b,
    {required String quoi}) {
  expect(a.widthFactor, b.widthFactor, reason: '$quoi : widthFactor');
  expect(a.showAuthorAvatar, b.showAuthorAvatar,
  ...
  expect(a.timestampFormat, b.timestampFormat,
...
void main() {
  ...
  group('🔴 SF-SKIN-G2 — avec skin : EXACTEMENT ce que le legacy fige', () {
    testWidgets('largeur 0.95, rayon 12 sur la REQUÊTE, avatar et nom masqués',
        (WidgetTester tester) async {
      final SfAIAssistView view =
          await _mount(tester, skin: const ZChatNotebookSkin());

      expect(view.requestMessageSettings.widthFactor, 0.95,
          reason: '`chatbot_conversation_screen.dart:3570`');
      expect(view.responseMessageSettings.widthFactor, 0.95,
      ...
      expect(view.requestMessageSettings.showAuthorName, isFalse,
      ...
      // AD-13 : le legacy écrit `BorderRadius.circular`, symétrique par
      // accident ; ici le type est DIRECTIONNEL, par décision.
      expect((shape as RoundedRectangleBorder).borderRadius,
    ...
    testWidgets('🔴 la bulle de RÉPONSE n\'a AUCUN `shape` — le legacy n\'en '
      ...
      final SfAIAssistView view =
          await _mount(tester, skin: const ZChatNotebookSkin());
      expect(view.responseMessageSettings.shape, isNull,
          reason: '🔴 un rayon a été INVENTÉ pour la réponse : le legacy ne '
    ...
    testWidgets('🔴 le FORMAT d\'horodatage legacy n\'est PAS imposé',
      ...
      final SfAIAssistView view =
          await _mount(tester, skin: const ZChatNotebookSkin());
      expect(view.requestMessageSettings.timestampFormat, isNull,
          reason: '🔴 `dd/MM/yyyy HH:mm:ss` est un format EU figé : l\'imposer '
      ...
      expect(ZChatNotebookReference.timestampFormatPattern,
          'dd/MM/yyyy HH:mm:ss');
    });
  });
  ...
  group('🔴 SF-SKIN-G3 — la chaîne des trois niveaux traverse la coquille', () {
    testWidgets('le JETON atteint le widget Syncfusion',
      ...
      expect(view.requestMessageSettings.widthFactor, 0.42,
    ...
    testWidgets('le PARAMÈTRE bat le jeton, jusque dans le widget',
      ...
      final SfAIAssistView view = await _mount(
        tester,
        skin: const ZChatNotebookSkin(bubbleWidthFactor: 0.33),
        theme: const ZcrudTheme(chatBubbleWidthFactor: 0.42),
      );
  ...
  group('🔴 SF-SKIN-G4 — la coquille ne perd RIEN de ce que le socle garantit',
      () {
