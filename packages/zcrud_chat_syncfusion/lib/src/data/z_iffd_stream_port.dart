/// Adaptateur de **port** : le fil textuel d'IFFD vu comme un `ZChatStreamPort`
/// — CHAT-6.
///
/// C'est la frontière proprement dite. Au-dessus de ce fichier, un hôte ne voit
/// que le contrat scellé du kernel (`Stream<ZResult<ZChatStreamEvent>>`) ; en
/// dessous, il n'y a que du texte et des sentinelles. Aucune compensation ne
/// traverse.
///
/// ## Ce que ce fichier NE fait PAS
///
/// Il ne parle **pas** au réseau. Aucun `dio`, aucun `http`, aucun `EventSource`
/// : le transport (en-têtes, App Check, `CancelToken`, `LineSplitter`, retrait
/// du préfixe `data: `) reste **côté app** — AD-11/AD-12, et c'est ce qui rend
/// ce paquet testable sans socket. L'hôte fournit un [ZIffdRawStreamOpener] ;
/// le port fait le reste.
library;
...
/// Ouvre le flux **brut** d'IFFD : des fragments de texte déjà décadrés du SSE.
///
/// L'implémentation appartient à l'hôte. Elle DOIT honorer l'annulation de
/// [token] (`token.whenCancelled`) — le port la traite aussi de son côté, mais
/// un transport qui l'ignore continuerait à consommer du quota serveur.
...
/// `ZChatStreamPort` adossé au fil textuel d'IFFD.
class ZIffdTextStreamPort implements ZChatStreamPort {
  ...
  Stream<ZResult<ZChatStreamEvent>> stream(
    ...
    try {
      ...
      // AD-10 : aucune exception du transport ne s'échappe du port. Elle devient
      // un `Left` typé — jamais un message de conversation (AD-5).
    ...
    if (completed) {
      ...
      // Arrêt VOULU : `cancelledByUser` sépare le geste de l'utilisateur d'une
      // coupure subie — les aplatir afficherait une erreur sur une annulation.
      yield Left<ZFailure, ZChatStreamEvent>(
        ZChatStreamInterruptedFailure(
          'cancelled',
          requestId: token.requestId,
...
/// Décode la réponse **non-streamée** d'IFFD (`{"data": …, "error": …,
/// "reasoning": …, "responseCode": …}` — `ai_models.dart:175-182`).
///
/// 🔴 Deux corrections de fond sur le client d'IFFD :
/// 1. `error` devient un **`Left` typé**, jamais un `data` affiché — IFFD
///    construit `AiResponse(error: …)` et l'ensemble de ses écrans le rend dans
///    la bulle de réponse.
/// 2. `reasoning` ne se retrouve **jamais** dans le corps : il reste hors des
///    blocs de contenu (le rendu de la trace est un canal distinct).
///
/// AD-10 : aucune entrée ne lève. Une map vide donne un unique bloc de texte
...
ZResult<List<ZContentBlock>> zIffdDecodeNonStreamResponse(
  ...
  // Le corps d'IFFD est du markdown brut : un unique `ZTextBlock`. Son rendu
  // riche relève d'un `ZChatRenderer` injecté, pas de ce décodeur.
...
///
/// AD-10 : un corps illisible devient un `Left` typé, jamais une exception.
