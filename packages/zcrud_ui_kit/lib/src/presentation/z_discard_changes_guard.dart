...
/// `ZFormController` (invariants AD-2, AD-13).
///
/// [ZDiscardChangesGuard] intercepte toute sortie tant qu'un formulaire est
/// *dirty*, sans jamais réinventer le dialogue de confirmation ni s'appuyer
/// sur un gestionnaire d'état :
/// * **`StatelessWidget` pur** — aucun gestionnaire d'état importé (ni
///   `flutter_riverpod`, ni `get`, ni `provider`), le type
///   `ValueListenable` provenant de `package:flutter/foundation.dart` (via
///   `material.dart`) ;
/// * l'état *dirty* est **consommé en lecture seule** via un
///   `ValueListenable<bool>` — canoniquement `ZFormController.isDirty`
///   (`zcrud_core`) — le garde n'a **aucune** poignée de mutation du
///   contrôleur ;
/// * la confirmation réutilise `showZConfirmDialog` (`ZConfirmTone`),
...
/// champ ni le `notifyListeners()` global, garantissant un rebuild
/// granulaire). L'appelant passe `controller.isDirty` : le contact avec le
/// cœur est **strictement en lecture** (aucune mutation, aucun import
/// `zcrud_core` requis ici).
///
/// [isDirty] est le **seul `bool`** de cette API — un prédicat strictement
/// binaire est l'exception assumée à la règle « enum plutôt que bool ». Le
/// résultat de la confirmation (discard / annuler) reste également binaire ;
/// la tonalité passe par l'enum [ZConfirmTone].
///
...
class ZDiscardChangesGuard extends StatelessWidget {
  ...
  /// une chaîne métier spécifique à une application (invariant AD-13).
  ...
  /// Sous-arbre protégé. **Non reconstruit** au flip *dirty*: il est
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder<bool>(
      ...
      // `child` passé par le paramètre → NON reconstruit au flip dirty.
      child: child,
