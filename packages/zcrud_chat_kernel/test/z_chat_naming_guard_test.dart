...
void main() {
  group('G16 — le faux-ami `WorkflowEffort` est INTERDIT (grep négatif)', () {
    test('0 occurrence de `WorkflowEffort` dans packages/*/lib', () {
      ...
      expect(
        files,
        isNotEmpty,
        reason: 'aucun fichier scanné : garde VACUELLE',
      );
      ...
      expect(
        ...
        isEmpty,
        reason:
            '🔴 FAUX-AMI RÉINTRODUIT. `WorkflowEffort` désigne DEUX '
    ...
    test('aucun symbole `*Effort*` dans le modèle de chat', () {
      ...
      for (final File f in chatDartFiles()) {
        ...
        for (final String raw in f.readAsLinesSync()) {
          ...
          // Une forme de catalogue (`route/catalog/`) est une DONNÉE : elle
          // nomme la clé d'effort d'un backend pour la traduire vers le
          // palier opaque `tier`, et vers RIEN d'autre. Seule cette entrée
          // d'alias, dont la cible est `'tier'`, est blanchie — la clé reste
          // un LITTÉRAL de forme, jamais un symbole du modèle.
          final String sansAlias = f.path.contains('/route/catalog/')
              ? reste.replaceAll(
                  RegExp(r"'[A-Za-z_]*[Ee]ffort[A-Za-z_]*'\s*:\s*'tier'"),
                  '',
                )
              : reste;
          if (sansAlias.contains('Effort')) {
            offenders.add('${f.path}:$no: $line');
          }
        }
      }
    ...
    test('l\'alias d\'effort d\'une forme de catalogue ne vise que `tier` '
        '(contre-preuve : toute autre cible rougit)', () {
      final RegExp alias = RegExp(
        r"'[A-Za-z_]*[Ee]ffort[A-Za-z_]*'\s*:\s*'tier'",
      );
      expect(alias.hasMatch("'workflowEffort': 'tier',"), isTrue);
      expect(alias.hasMatch("'workflow_effort': 'tier',"), isTrue);
      expect(alias.hasMatch("'workflowEffort': 'compute_effort',"), isFalse);
      expect(alias.hasMatch("final String workflowEffort = 'tier';"), isFalse);
      int seen = 0;
      for (final File f in chatDartFiles()) {
        if (!f.path.contains('/route/catalog/')) continue;
        for (final String raw in f.readAsLinesSync()) {
          final String line = stripComment(raw).replaceAll(
            RegExp(r'\b(ZChatComputeEffort|computeEffort|compute_effort)\b'),
            '',
          );
          if (!line.contains('ffort')) continue;
          seen++;
          expect(
            alias.hasMatch(line),
            isTrue,
            reason: 'clé d\'effort hors alias vers `tier` : $line',
          );
        }
      }
      expect(
        seen,
        greaterThan(0),
        reason: 'la forme `lex` doit traduire l\'effort du backend en `tier`',
      );
    });
    ...
    test('la dartdoc de `ZChatResponseLength` cite les DEUX origines', () {
      ...
      expect(
        src,
        contains('domain/enums/chat_enums.dart:20-26'),
        reason: 'le fichier d\'origine LEX doit être cité nommément',
      );
      expect(
        src,
        contains('lib/src/domain/models/ai/ai_models.dart:119-122'),
        reason: 'le fichier d\'origine IFFD doit être cité nommément',
      );
      expect(
        src,
        contains('ZChatComputeEffort'),
    ...
    test('CHAT-1 — le nom réservé est HONORÉ : `ZChatComputeEffort` existe, '
      ...
      expect(
        src,
        contains('backend/app/models/chat.py:261'),
        reason: 'l\'origine LEX de l\'axe calcul doit être citée',
      );
      expect(
        src,
        contains('shared/schemas/base_request.py:104'),
      ...
      for (final String verbosite in <String>[
        'concis',
        'detaille',
        'standard',
      ]) {
        expect(
          src.contains("'$verbosite'"),
          isFalse,
    ...
    test('CHAT-1b — G16 RETENDUE : les deux axes restent NON CONFONDABLES sur '
      ...
      final File port = File('${domain.path}/ai/z_chat_generation_port.dart');
      ...
      expect(
        portCode,
        contains('final ZChatResponseLength? responseLength;'),
        reason:
            '🔴 l\'axe VERBOSITÉ a disparu de la requête — absorbé par '
            'l\'axe calcul ?',
      );
      expect(
        portCode,
        contains('final ZChatComputeEffort? computeEffort;'),
        reason:
            '🔴 l\'axe CALCUL a disparu de la requête — absorbé par la '
            'verbosité ?',
      );
      ...
      for (final String etranger in <String>[
        ...
        expect(
          calculCode.contains(etranger),
          isFalse,
          reason:
              '🔴 `$etranger` (VERBOSITÉ) référencé dans le type de '
      ...
      expect(
        enumsCode.contains('ZChatComputeEffort'),
        isFalse,
        reason:
            '🔴 le type de l\'axe CALCUL référencé dans le CODE des '
      ...
      expect(
        calculCode,
        contains('final int level;'),
        reason:
            '🔴 l\'axe calcul n\'est plus l\'entier 1..5 commun aux deux '
            'backends',
      );
      expect(
        enumsCode,
        contains('enum ZChatResponseLength'),
  ...
  group(
    'G17 — pureté & neutralité de `zcrud_chat_kernel/lib/` — périmètre ÉLARGI en fin d\'epic (le barrel était hors scan)',
    () {
      ...
      test('aucun codegen : ni `part` GÉNÉRÉ, ni annotation de sérialisation '
        ...
        for (final File f in chatDartFiles()) {
          ...
          for (final String raw in f.readAsLinesSync()) {
            ...
            if (partDecl.firstMatch(line) case final RegExpMatch m) {
              ...
              final bool existe = File('${f.parent.path}/$uri').existsSync();
              if (genere || !existe) {
                offenders.add('${f.path}:$no: $line');
              }
              continue;
        ...
        expect(
          ...
          isEmpty,
          reason:
              'ni `zcrud_core` ni `zcrud_chat_kernel` n\'ont de codegen '
              '(D1) :\n${offenders.join('\n')}',
        );
      ...
      test('AC2 — `pubspec.yaml` de `zcrud_core` ET de `zcrud_chat_kernel` '
        ...
        for (final String pkg in <String>['zcrud_core', 'zcrud_chat_kernel']) {
          ...
          final String pubspec =
              File('${repoRoot().path}/packages/$pkg/pubspec.yaml')
                  .readAsLinesSync()
          ...
          for (final String interdit in <String>[
            ...
            expect(
              pubspec.contains(interdit),
              isFalse,
              reason:
                  '`$interdit` déclaré dans $pkg ⇒ codegen réintroduit '
                  '(D1) / AD-1',
            );
          }
        }
      ...
      test('AC11 — les deux entités citent `ZSyncMeta.reservedKeys` (règle (B) '
        ...
        for (final String nom in <String>[
          ...
          // Troisième entité `ZExtensible` écrite à la main du kernel : le
          // routeur du catalogue de routes, même règle (B).
          'route/z_chat_router.dart',
        ]) {
      ...
      test('AC1 (révisé CHAT-0r) — le module chat est exporté par SON kernel, '
        ...
        expect(
          barrel.contains('src/domain/chat/'),
          isFalse,
        ...
        expect(
          ...
          isFalse,
          reason:
              'les helpers JSON doivent vivre dans `src/domain/json/`, '
