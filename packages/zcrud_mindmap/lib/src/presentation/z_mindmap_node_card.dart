/// Carte de nœud thématisée partagée par le graphe et la vue liste.
///
...
/// Rendu **compact** : `label` seul, mono-ligne, thématisé. Ne
...
/// Bornage d'une cellule du **graphe** : un contenu de nœud
...
/// overflow` et **jamais** de mesure intrinsèque qui déborderait.
///
...
/// débordement peint. Aligné en `topStart` (RTL-safe, invariant AD-13).
///
/// **N'est PAS appliqué à la vue liste / l'outline** : là, le rendu riche
/// **complet** (non borné) reste garanti. Seule la cellule du graphe borne.
///
/// Pouvoir discriminant : sans ce bornage, un contenu plus haut que la
    ...
    return ClipRect(
      child: SizedBox.fromSize(
        ...
        child: OverflowBox(
          ...
          // overflow) ; le ClipRect tronque le surplus.
...
class ZMindmapNodeCard extends StatelessWidget {
  ...
  /// Mode **compact** : rendu **condensé label-seul** — le
  /// [contentBuilder] (et donc tout extrait/contenu long ou rich-text) est
  /// **masqué**, seul `label` est rendu en texte brut mono-ligne. Défaut
  /// `false` ⇒ rendu inchangé pour un hôte passif. La garde de masquage vit
  /// ici : la retirer (toujours appeler [contentBuilder]) laisserait le
  /// contenu visible même en mode compact.
  final bool compact;
  ...
  /// (évite le double-abonnement O(n), invariant AD-2).
      ...
      child: DecoratedBox(
        decoration: BoxDecoration(
          ...
          border: Border.all(
            ...
            // Épaisseur injectable par état. Le repli sur les
            // constantes historiques (1 / 2) garde le rendu strictement
            // inchangé quand l'hôte n'injecte rien.
            width: isSelected
