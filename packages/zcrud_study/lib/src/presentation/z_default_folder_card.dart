...
/// (**CR-IFFD-64**), sixième et dernier rendu par défaut de la famille.
///
...
/// hôte re-décorait avec ses propres constantes. L'argument décisif est celui
/// du propriétaire d'IFFD : *« le `ZDefaultFolderCard` est appelé à évoluer en
/// intégrant les futures améliorations possibles du design et fonctionnalité
...
/// **BRUTE**, et le port HSL du socle (`zReadableTypeTint`) ne bornait que la
/// *clarté HSL* — pas le contraste : mesuré, `#FFFF00` rendait **2.13:1** et
...
/// ## Invariants
///
/// - **FR-26/NFR-S7** : aucune couleur ni libellé en dur ; tout texte visible
///   est **injecté** ; `null` ⇒ **absent** de l'arbre (AD-4), jamais un
///   `SizedBox.shrink()` inerte.
/// - **AD-13** : insets et alignements **directionnels**, cible ≥ 48 dp
...
/// - **AD-2/SM-1** : `StatelessWidget` pur, `const` partout où c'est possible,
///   aucun état détenu, aucune liste matérialisée.
/// - **AD-10** : chaîne de repli **totale** — une clé de couleur inconnue ne
...
/// carte — le socle ne peut ni composer ni accorder ce libellé (FR-26). Les
/// deux ne sont pas interchangeables ; les confondre obligerait la carte à
/// fabriquer du texte.
@immutable
class ZFolderCardCount {
  ...
  /// la couleur du badge n'en est qu'un rappel (AD-13).
  final String label;
...
/// Carte de dossier d'étude **par défaut** du socle (CR-IFFD-64).
///
/// ```dart
/// ZDefaultFolderCard(
///   title: folder.name,
///   subtitle: subjectNameOf(folder),          // OPTION (AD-4)
...
class ZDefaultFolderCard extends StatelessWidget {
  ...
  /// zone **absente** (AD-4). Ignoré si [countsSlot] est fourni.
  ...
  /// **un badge textuel** et enrichit l'annonce ; la teinte et le liseré sont
  ...
  /// puis référence (4). Une valeur ≤ 0 **supprime** la bande (AD-4 : absente
  /// de l'arbre, jamais une bande de hauteur nulle).
  ...
  Widget build(BuildContext context) {
    ...
    // AD-4 — une bande de hauteur nulle serait un nœud inerte : elle est
    // ABSENTE de l'arbre.
    final Widget? band =
        accent ??
    ...
    return ZFolderCard(
      ...
      // même palette (chaîne totale, AD-10).
  ...
  /// (qui priment). Couleur = **rôle** `shadowColor` : le legacy y écrivait
  /// `Colors.black` en dur, jamais reproduit ici (FR-26).
  ...
  /// porte déjà l'information, la tuile n'est qu'un rappel visuel (AD-13).
  ...
  /// **absent** de l'arbre (AD-4) — jamais un espace réservé.
  Widget? _buildBelowSubtitle(
    ...
    // Le legacy peint la couleur du dossier @ 0.8 : on part de cette
    ...
    // Leçon CR-IFFD-37/CR-IFFD-56 : dans un slot prêté en fit LOOSE, des
    // enfants INFLEXIBLES s'AJOUTENT à la hauteur au lieu d'y participer —
    // chaque bloc est donc `Flexible`, l'espacement compris.
    return Column(
  ...
  /// Zone de badges de compteur. `null` ⇒ **absente** (AD-4).
  Widget? _buildCounts(
    BuildContext context,
    ZcrudTheme theme,
    ZColorPair pair,
    ...
    // Défilement horizontal (patron legacy) : une rangée de badges ne déborde
