/// Enrôlement des champs spécialisés `zcrud_field_extras` dans un
/// `ZWidgetRegistry` — patron `registerZMediaFieldWidgets`/`registerZHtmlFields`.
///
...
/// `zcrud_core`, aucune dépendance lourde tirée — invariant AD-1).
///
...
///
/// **« Tags riches » non enrôlé** : `EditionFieldType.tags` route vers la
/// famille NATIVE `tags` (pas `registryOrFallback`) — un `kind == 'tags'`
/// serait du code mort. Le besoin est déjà couvert zéro-dépendance par
/// `ZSubListDisplayMode.tags`. Un chemin dispatcher-atteignable exigerait un
/// nouveau type d'enum cœur (`richTags`).
library;
...
///
/// [onBuild] est un hook de test partagé propagé aux trois widgets.
