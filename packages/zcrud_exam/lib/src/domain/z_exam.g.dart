...
ZExam _$ZExamFromMap(Map<String, dynamic> map) => ZExam(
  ...
  date: _$asDateTime(map['date']),
  reminderEnabled: map['reminder_enabled'] is bool
      ? map['reminder_enabled'] as bool
      : false,
  reminderDaysBefore: map['reminder_days_before'] is List
      ? (map['reminder_days_before'] as List).whereType<int>().toList()
      : const <int>[],
);

extension ZExamZcrud on ZExam {
...
const List<ZFieldSpec> $ZExamFieldSpecs = <ZFieldSpec>[
  ...
  ZFieldSpec(name: 'title', type: EditionFieldType.text, label: 'Examen'),
  ZFieldSpec(name: 'date', type: EditionFieldType.dateTime),
  ZFieldSpec(name: 'reminder_enabled', type: EditionFieldType.boolean),
  ZFieldSpec(
...
/// Clés que `ZExam.toMap()` PEUT produire — surensemble
...
void registerZExam(ZcrudRegistry registry) {
  ...
  _$zRequireExtraPreserved<ZExam>(
    'ZExam',
    ZExam.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZExam>(
    'exam',
...
const Set<String> $ZExamTimestampFields = <String>{};
