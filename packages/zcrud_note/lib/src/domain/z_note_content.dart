...
/// neutres** (`List<Map<String, dynamic>>`),.
///
...
/// - un consommateur legacy persiste `content` en `String?` qui est **tantôt du Delta JSON, tantôt
///   du markdown** ;
...
///
/// ## DEUX politiques, DEUX fonctions (remédiation, code-review
/// )
///
...
///
/// ## Ce n'est **PAS** une heuristique (AD-28), ni un `ZCodec`
///
...
///   **littéralement** le code d'un consommateur legacy (répété **verbatim en 4 sites** :
...
// `domain.dart`, JAMAIS `zcrud_core.dart` (AD-14) : le barrel COMPLET du cœur
...
// (Mesuré pendant : import du barrel complet ⇒ « [gate:web] ÉCHEC : la
...
/// | `List` **PARTIELLEMENT** valide (`[{insert: …}, {retain: 1}]`, `[{insert: …}, null]`) | **les ops portant `insert` SURVIVENT** ; l'élément parasite est **écarté**. **JAMAIS `[]`** |
/// | `List` portant une `String` (fragment texte) | l'élément devient `{'insert': '<txt>\n'}` — **le texte SURVIT** |
/// | `String` qui **parse** en JSON `List` d'ops **TOUTES** valides | ops décodées (compat `ZDeltaCodec.encode` / corpus Delta un consommateur legacy) |
/// | `String` **non-Delta** non vide (markdown lex, sticky-note un consommateur legacy, texte plat, `'[1,2]'`, `'{}'`) | **`[{'insert': '<raw>\n'}]` — le texte SURVIT VERBATIM** |
/// | `String` vide / blanche | `[]` |
...
/// texte) ne rend `[]`. C'est la règle, et elle vaut désormais sur **les
...
List<Map<String, dynamic>> normalizeNoteContentOps(Object? raw) {
  ...
  if (raw is List) {
    // branche PRÉSERVANTE : on sauve TOUT ce qui est sauvable.
  ...
  if (raw is String) {
    ...
    //
    // ICI la validation DOIT être STRICTE (tout-ou-rien) : elle ne « valide »
    ...
    // LE POINT LE PLUS IMPORTANT DU PACKAGE : ce n'est pas du Delta, donc
    ...
    // EXPLICITE — jamais une devinette faite ici.
...
///
/// gel **PROFOND** (liste + chaque op + valeurs
...
///
/// **Ne JAMAIS l'utiliser sur la branche `List` native** : elle y détruirait
/// les ops valides d'une liste partiellement corrompue (). Sa rigueur
...
///
/// C'est la remédiation de : une **seule** op parasite ne peut plus
...
List<Map<String, dynamic>> _coerceOpsPreserving(List<Object?> raw) {
  ...
  for (final op in raw) {
    ...
    if (op is String) {
      // Un fragment TEXTE dans la liste : s'applique — on ne détruit pas du
...
///
/// 🟡 **MEDIUM-1 (code-review)** — l'argument écrit pour [noteContentEquals]
...
/// un consommateur legacy). Il n'y avait **pas** été appliqué : `_mapEquals` était **superficiel** ⇒
...
///
/// **Le défaut était SYSTÉMIQUE** : le même `_mapEquals`/`_mapHash`
/// **superficiel** était copié dans `zcrud_document`, `zcrud_flashcard` et
/// `zcrud_study_kernel` ⇒ dette, **SOLDÉE par**.
///
/// ## cette fonction est désormais un **ALIAS DÉLÉGANT**
///
