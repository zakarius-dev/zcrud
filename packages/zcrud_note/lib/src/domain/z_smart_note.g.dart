...
ZSmartNote _$ZSmartNoteFromMap(Map<String, dynamic> map) => ZSmartNote(
...
extension ZSmartNoteZcrud on ZSmartNote {
...
const List<ZFieldSpec> $ZSmartNoteFieldSpecs = <ZFieldSpec>[
  ...
  ZFieldSpec(name: 'title', type: EditionFieldType.text, label: 'Titre'),
  ZFieldSpec(name: 'created_at', type: EditionFieldType.dateTime),
];

/// Clés que `ZSmartNote.toMap()` PEUT produire — surensemble
...
void registerZSmartNote(ZcrudRegistry registry) {
  ...
  _$zRequireExtraPreserved<ZSmartNote>(
    'ZSmartNote',
    ZSmartNote.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZSmartNote>(
    'smart_note',
