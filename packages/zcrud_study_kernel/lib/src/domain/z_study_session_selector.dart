/// Primitive pure de sélection de session.
///
/// Applique les filtres d'une [ZStudySessionConfig] à une collection de
/// candidats [ZSessionCandidate] (port neutre — flashcards, notes,
/// mindmaps…) et retourne la sélection déterministe. Pur-Dart, sans état,
/// sans I/O ni horloge : aucune notion de progression runtime de session.
/// Opérer sur le port [ZSessionCandidate] plutôt que sur une entité concrète
/// préserve l'acyclicité (invariant AD-1).
///
/// Sémantique : les filtres se composent en **ET** (dossier ∧ tags ∧ types)
/// puis le plafond `count` s'applique :
/// - `folderId == null` ⇒ pas de filtre dossier ; sinon ne retient que les
///   candidats dont `folderId == config.folderId` **ou** `subFolderId ==
///   config.folderId` (le dossier cible couvre ses sous-dossiers —
///   rattachement inverse 2 niveaux) ;
...
/// Sélecteur pur de candidats de session (enveloppe une
/// [ZStudySessionConfig]).
///
...
class ZStudySessionSelector {
  ...
  /// un prédicat par candidat — voir [selectFrom]).
  bool matches(ZSessionCandidate candidate) =>
      _matchesFolder(candidate) &&
      _matchesTags(candidate) &&
      _matchesTypes(candidate);

  /// Retourne la sélection ordonnée : les [candidates] filtrées par
  /// [matches] puis tronquées à `config.count` (illimité si `null`, vide si
  /// `<= 0`).
  ///
  /// Générique sur `T extends ZSessionCandidate` : le type concret d'entrée
  /// est préservé en sortie (un satellite récupère ses propres entités, pas
  /// des `ZSessionCandidate` opaques). L'ordre d'entrée est préservé ; la
