...
ZStudySubject _$ZStudySubjectFromMap(Map<String, dynamic> map) => ZStudySubject(
...
extension ZStudySubjectZcrud on ZStudySubject {
...
const List<ZFieldSpec> $ZStudySubjectFieldSpecs = <ZFieldSpec>[
  ...
  ZFieldSpec(name: 'label', type: EditionFieldType.text, label: 'Matière'),
  ZFieldSpec(name: 'color_key', type: EditionFieldType.text),
];

/// Clés que `ZStudySubject.toMap()` PEUT produire — surensemble
...
const Set<String> $ZStudySubjectPersistedKeys = <String>{
  'id',
  'organization_id',
  'kind',
  'code',
...
/// Enregistre `ZStudySubject` (kind "study_subject") sur [registry] : (dé)sérialisation + schéma.
void registerZStudySubject(ZcrudRegistry registry) {
  // DW-ES14-1 (AD-4) : POUVOIR observé, pas seulement signature vérifiée.
  _$zRequireExtraPreserved<ZStudySubject>(
    'ZStudySubject',
    ZStudySubject.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZStudySubject>(
    'study_subject',
    fromMap: ZStudySubject.fromMap,
    toMap: (value) => value.toMap(),
    fieldSpecs: $ZStudySubjectFieldSpecs,
    fromMapWithContext: (map, context) => ZStudySubject.fromMap(
      map,
      extensionParser: context?.extensionParser == null
          ? null
          : (json) => context!.extensionParser!('study_subject', json),
    ),
  );
}
...
const Set<String> $ZStudySubjectTimestampFields = <String>{};
