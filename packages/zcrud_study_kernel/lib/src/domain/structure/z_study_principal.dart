/// Entité `ZStudyPrincipal` — mandant : ce à quoi on attribue une possession,
/// une participation ou un droit.
///
/// Un mandant n'est pas nécessairement une personne : [kind] est une chaîne
/// opaque (`'user'`, `'group'`, `'service'` sont les valeurs usuelles, aucune
/// n'est imposée ni interprétée). Le noyau ne porte **aucune** donnée
/// d'identité (pas d'adresse, pas de contact) : un mandant est un identifiant
/// opaque, un libellé d'affichage et une clé d'avatar résolue côté hôte.
///
/// **Slots d'extension (invariant AD-4)** : `ZExtensible` → `extra` +
/// `extension`. **Décodage défensif (invariant AD-10)** : `fromMap` ne lève
/// jamais.
library;

import 'package:zcrud_annotations/zcrud_annotations.dart';
import 'package:zcrud_core/domain.dart';

import 'z_external_ref.dart';
import 'z_study_json.dart';

part 'z_study_principal.g.dart';

/// Reconstruit une [ZExtension] concrète depuis sa map JSON, ou `null`.
typedef ZStudyPrincipalExtensionParser =
    ZExtension? Function(Map<String, dynamic> json);

/// Mandant immuable.
@ZcrudModel(kind: 'study_principal', fieldRename: ZFieldRename.snake)
class ZStudyPrincipal extends ZEntity with ZExtensible {
  /// Construit un mandant.
  const ZStudyPrincipal({
    this.id,
    this.kind = '',
    this.label,
    this.avatarKey,
    this.externalRefs = const <ZExternalRef>[],
    this.extension,
    Map<String, dynamic> extra = const <String, dynamic>{},
    // ignore: prefer_initializing_formals
  }) : _extra = extra;

  /// Reconstruit défensivement depuis une map persistée (invariant AD-10).
  factory ZStudyPrincipal.fromMap(
    Map<String, dynamic> map, {
    ZStudyPrincipalExtensionParser? extensionParser,
  }) {
    final base = _$ZStudyPrincipalFromMap(map);
    return ZStudyPrincipal(
      id: base.id,
      kind: base.kind,
      label: base.label,
      avatarKey: base.avatarKey,
      externalRefs: zStudyDecodeExternalRefs(map['external_refs']),
      extension: zDecodeExtension(map['extension'], extensionParser),
      extra: _sanitizeExtra(map),
    );
  }

  /// Identité opaque (nullable pour l'éphémère).
  @override
  @ZcrudId()
  final String? id;

  /// Nature du mandant — chaîne opaque, défaut `''`.
  @ZcrudField()
  final String kind;

  /// Libellé affichable, `null` si non déclaré.
  @ZcrudField()
  final String? label;

  /// Clé d'avatar résolue côté hôte (jamais une URL ni un binaire), `null` si
  /// absente.
  @ZcrudField()
  final String? avatarKey;

  /// Identifiants du mandant dans des systèmes tiers, défaut `const []`.
  @ZcrudIgnore()
  final List<ZExternalRef> externalRefs;

  /// Slot type additif versionné (invariant AD-4), `null` si absent.
  @override
  final ZExtension? extension;

  final Map<String, dynamic> _extra;

  /// Échappatoire non typée (invariant AD-4), défaut `const {}`.
  @override
  Map<String, dynamic> get extra => zNormalizeExtra(_extra, _reservedKeys);

  /// Sérialise vers la map persistée (snake_case) ; aucune valeur absente
  /// n'écrit de clé.
  Map<String, dynamic> toMap() {
    final map = <String, dynamic>{
      ...extra,
      ...zStudyPrune(ZStudyPrincipalZcrud(this).toMap()),
    };
    if (externalRefs.isNotEmpty) {
      map['external_refs'] = zStudyEncodeList(
        externalRefs,
        (ZExternalRef ref) => ref.toMap(),
      );
    }
    if (extension != null) map['extension'] = extension!.toJson();
    return map;
  }

  /// Copie à sentinelle (un argument omis préserve la valeur, `null` explicite
  /// remet à `null`).
  ZStudyPrincipal copyWith({
    Object? id = _$undefined,
    Object? kind = _$undefined,
    Object? label = _$undefined,
    Object? avatarKey = _$undefined,
    Object? externalRefs = _$undefined,
    Object? extension = _$undefined,
    Object? extra = _$undefined,
  }) => ZStudyPrincipal(
    id: identical(id, _$undefined) ? this.id : id as String?,
    kind: identical(kind, _$undefined) ? this.kind : kind as String,
    label: identical(label, _$undefined) ? this.label : label as String?,
    avatarKey: identical(avatarKey, _$undefined)
        ? this.avatarKey
        : avatarKey as String?,
    externalRefs: identical(externalRefs, _$undefined)
        ? this.externalRefs
        : externalRefs as List<ZExternalRef>,
    extension: identical(extension, _$undefined)
        ? this.extension
        : extension as ZExtension?,
    extra: identical(extra, _$undefined)
        ? this.extra
        : _sanitizeExtra(extra as Map<String, dynamic>),
  );

  /// Clés persistées réservées (schéma généré + canaux manuels + `ZSyncMeta`).
  static final Set<String> _reservedKeys = <String>{
    for (final spec in $ZStudyPrincipalFieldSpecs) spec.name,
    'external_refs',
    'extension',
    ...ZSyncMeta.reservedKeys,
  };

  static Map<String, dynamic> _sanitizeExtra(Map<String, dynamic> raw) =>
      zSanitizeExtra(raw, _reservedKeys);

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is ZStudyPrincipal &&
          id == other.id &&
          kind == other.kind &&
          label == other.label &&
          avatarKey == other.avatarKey &&
          zStudyListEquals(externalRefs, other.externalRefs) &&
          extension == other.extension &&
          zJsonEquals(extra, other.extra);

  @override
  int get hashCode => Object.hash(
    id,
    kind,
    label,
    avatarKey,
    Object.hashAll(externalRefs),
    extension,
    zJsonHash(extra),
  );
}
