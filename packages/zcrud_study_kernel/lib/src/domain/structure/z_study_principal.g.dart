...
part of 'z_study_principal.dart';
...
ZStudyPrincipal _$ZStudyPrincipalFromMap(Map<String, dynamic> map) =>
    ZStudyPrincipal(
      ...
      avatarKey: map['avatar_key'] is String
          ? map['avatar_key'] as String
          : null,
    );

extension ZStudyPrincipalZcrud on ZStudyPrincipal {
...
const List<ZFieldSpec> $ZStudyPrincipalFieldSpecs = <ZFieldSpec>[
  ...
  ZFieldSpec(name: 'avatar_key', type: EditionFieldType.text),
];

/// Clés que `ZStudyPrincipal.toMap()` PEUT produire — surensemble
...
const Set<String> $ZStudyPrincipalPersistedKeys = <String>{
  'id',
  'kind',
  'label',
  'avatar_key',
...
/// Enregistre `ZStudyPrincipal` (kind "study_principal") sur [registry] : (dé)sérialisation + schéma.
void registerZStudyPrincipal(ZcrudRegistry registry) {
  // DW-ES14-1 (AD-4) : POUVOIR observé, pas seulement signature vérifiée.
  _$zRequireExtraPreserved<ZStudyPrincipal>(
    'ZStudyPrincipal',
    ZStudyPrincipal.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZStudyPrincipal>(
    'study_principal',
    fromMap: ZStudyPrincipal.fromMap,
    toMap: (value) => value.toMap(),
    fieldSpecs: $ZStudyPrincipalFieldSpecs,
    fromMapWithContext: (map, context) => ZStudyPrincipal.fromMap(
      map,
      extensionParser: context?.extensionParser == null
          ? null
          : (json) => context!.extensionParser!('study_principal', json),
    ),
  );
}
...
const Set<String> $ZStudyPrincipalTimestampFields = <String>{};
