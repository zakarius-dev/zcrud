...
ZStudyPeriod _$ZStudyPeriodFromMap(Map<String, dynamic> map) => ZStudyPeriod(
  id: map['id'] is String ? map['id'] as String : null,
  calendarId: map['calendar_id'] is String
      ? map['calendar_id'] as String
      : null,
...
extension ZStudyPeriodZcrud on ZStudyPeriod {
...
/// `toMap()`/`copyWith()` de `ZStudyPeriod` en MEMBRES D'INSTANCE.
///
/// À appliquer (`class ZStudyPeriod … with _$ZStudyPeriodZcrud`) quand un membre
...
const List<ZFieldSpec> $ZStudyPeriodFieldSpecs = <ZFieldSpec>[
  ZFieldSpec(name: 'id', type: EditionFieldType.text, isId: true),
  ZFieldSpec(name: 'calendar_id', type: EditionFieldType.text),
...
/// Clés que `ZStudyPeriod.toMap()` PEUT produire — surensemble
...
const Set<String> $ZStudyPeriodPersistedKeys = <String>{
  'id',
  'calendar_id',
  'parent_id',
  'kind',
...
/// Enregistre `ZStudyPeriod` (kind "study_period") sur [registry] : (dé)sérialisation + schéma.
void registerZStudyPeriod(ZcrudRegistry registry) {
  // DW-ES14-1 (AD-4) : POUVOIR observé, pas seulement signature vérifiée.
  _$zRequireExtraPreserved<ZStudyPeriod>(
    'ZStudyPeriod',
    ZStudyPeriod.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZStudyPeriod>(
    'study_period',
    fromMap: ZStudyPeriod.fromMap,
    toMap: (value) => value.toMap(),
    fieldSpecs: $ZStudyPeriodFieldSpecs,
    fromMapWithContext: (map, context) => ZStudyPeriod.fromMap(
      map,
      extensionParser: context?.extensionParser == null
          ? null
          : (json) => context!.extensionParser!('study_period', json),
    ),
  );
}
...
const Set<String> $ZStudyPeriodTimestampFields = <String>{};
