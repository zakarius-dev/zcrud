...
ZStudyOrgUnit _$ZStudyOrgUnitFromMap(Map<String, dynamic> map) => ZStudyOrgUnit(
...
extension ZStudyOrgUnitZcrud on ZStudyOrgUnit {
...
/// Clés que `ZStudyOrgUnit.toMap()` PEUT produire — surensemble
...
const Set<String> $ZStudyOrgUnitPersistedKeys = <String>{
  'id',
  'organization_id',
  'parent_id',
  'kind',
...
/// Enregistre `ZStudyOrgUnit` (kind "study_org_unit") sur [registry] : (dé)sérialisation + schéma.
void registerZStudyOrgUnit(ZcrudRegistry registry) {
  // DW-ES14-1 (AD-4) : POUVOIR observé, pas seulement signature vérifiée.
  _$zRequireExtraPreserved<ZStudyOrgUnit>(
    'ZStudyOrgUnit',
    ZStudyOrgUnit.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZStudyOrgUnit>(
    'study_org_unit',
    fromMap: ZStudyOrgUnit.fromMap,
    toMap: (value) => value.toMap(),
    fieldSpecs: $ZStudyOrgUnitFieldSpecs,
    fromMapWithContext: (map, context) => ZStudyOrgUnit.fromMap(
      map,
      extensionParser: context?.extensionParser == null
          ? null
          : (json) => context!.extensionParser!('study_org_unit', json),
    ),
  );
}
...
const Set<String> $ZStudyOrgUnitTimestampFields = <String>{};
