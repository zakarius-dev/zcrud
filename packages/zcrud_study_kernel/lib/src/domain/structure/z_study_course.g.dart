...
ZStudyCourse _$ZStudyCourseFromMap(Map<String, dynamic> map) => ZStudyCourse(
  ...
  expectedHours: _$asDouble(map['expected_hours']),
);

extension ZStudyCourseZcrud on ZStudyCourse {
...
/// `toMap()`/`copyWith()` de `ZStudyCourse` en MEMBRES D'INSTANCE.
///
/// À appliquer (`class ZStudyCourse … with _$ZStudyCourseZcrud`) quand un membre
...
const List<ZFieldSpec> $ZStudyCourseFieldSpecs = <ZFieldSpec>[
  ...
  ZFieldSpec(name: 'expected_hours', type: EditionFieldType.float),
];

/// Clés que `ZStudyCourse.toMap()` PEUT produire — surensemble
...
const Set<String> $ZStudyCoursePersistedKeys = <String>{
  'id',
  'organization_id',
  'subject_id',
  'kind',
...
/// Enregistre `ZStudyCourse` (kind "study_course") sur [registry] : (dé)sérialisation + schéma.
void registerZStudyCourse(ZcrudRegistry registry) {
  // DW-ES14-1 (AD-4) : POUVOIR observé, pas seulement signature vérifiée.
  _$zRequireExtraPreserved<ZStudyCourse>(
    'ZStudyCourse',
    ZStudyCourse.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZStudyCourse>(
    'study_course',
    fromMap: ZStudyCourse.fromMap,
    toMap: (value) => value.toMap(),
    fieldSpecs: $ZStudyCourseFieldSpecs,
    fromMapWithContext: (map, context) => ZStudyCourse.fromMap(
      map,
      extensionParser: context?.extensionParser == null
          ? null
          : (json) => context!.extensionParser!('study_course', json),
    ),
  );
}
...
const Set<String> $ZStudyCourseTimestampFields = <String>{};
