...
const List<ZFieldSpec> $ZStudyCalendarFieldSpecs = <ZFieldSpec>[
  ...
  ZFieldSpec(name: 'timezone', type: EditionFieldType.text),
  ZFieldSpec(name: 'label', type: EditionFieldType.text),
  ZFieldSpec(name: 'kind', type: EditionFieldType.text),
];

/// Clés que `ZStudyCalendar.toMap()` PEUT produire — surensemble
...
const Set<String> $ZStudyCalendarPersistedKeys = <String>{
  'id',
  'organization_id',
  'timezone',
  'label',
...
/// Enregistre `ZStudyCalendar` (kind "study_calendar") sur [registry] : (dé)sérialisation + schéma.
void registerZStudyCalendar(ZcrudRegistry registry) {
  // DW-ES14-1 (AD-4) : POUVOIR observé, pas seulement signature vérifiée.
  _$zRequireExtraPreserved<ZStudyCalendar>(
    'ZStudyCalendar',
    ZStudyCalendar.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZStudyCalendar>(
    'study_calendar',
    fromMap: ZStudyCalendar.fromMap,
    toMap: (value) => value.toMap(),
    fieldSpecs: $ZStudyCalendarFieldSpecs,
    fromMapWithContext: (map, context) => ZStudyCalendar.fromMap(
      map,
      extensionParser: context?.extensionParser == null
          ? null
          : (json) => context!.extensionParser!('study_calendar', json),
    ),
  );
}
...
const Set<String> $ZStudyCalendarTimestampFields = <String>{};
