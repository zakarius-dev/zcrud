...
part of 'z_study_program_course.dart';
...
ZStudyProgramCourse _$ZStudyProgramCourseFromMap(Map<String, dynamic> map) =>
    ZStudyProgramCourse(
      id: map['id'] is String ? map['id'] as String : null,
      programId: map['program_id'] is String ? map['program_id'] as String : '',
      courseId: map['course_id'] is String ? map['course_id'] as String : '',
      periodPattern: map['period_pattern'] is String
          ? map['period_pattern'] as String
          : null,
      isRequired: map['is_required'] is bool
          ? map['is_required'] as bool
          : false,
      credits: _$asDouble(map['credits']),
      coefficient: _$asDouble(map['coefficient']),
      order: _$asInt(map['order']),
    );

extension ZStudyProgramCourseZcrud on ZStudyProgramCourse {
...
const List<ZFieldSpec> $ZStudyProgramCourseFieldSpecs = <ZFieldSpec>[
  ...
  ZFieldSpec(name: 'program_id', type: EditionFieldType.text),
  ZFieldSpec(name: 'course_id', type: EditionFieldType.text),
  ZFieldSpec(name: 'period_pattern', type: EditionFieldType.text),
  ZFieldSpec(name: 'is_required', type: EditionFieldType.boolean),
  ZFieldSpec(name: 'credits', type: EditionFieldType.float),
  ZFieldSpec(name: 'coefficient', type: EditionFieldType.float),
  ZFieldSpec(name: 'order', type: EditionFieldType.integer),
];

/// Clés que `ZStudyProgramCourse.toMap()` PEUT produire — surensemble
...
const Set<String> $ZStudyProgramCoursePersistedKeys = <String>{
  'id',
  'program_id',
  'course_id',
  'period_pattern',
...
/// Enregistre `ZStudyProgramCourse` (kind "study_program_course") sur [registry] : (dé)sérialisation + schéma.
void registerZStudyProgramCourse(ZcrudRegistry registry) {
  // DW-ES14-1 (AD-4) : POUVOIR observé, pas seulement signature vérifiée.
  _$zRequireExtraPreserved<ZStudyProgramCourse>(
    'ZStudyProgramCourse',
    ZStudyProgramCourse.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZStudyProgramCourse>(
    'study_program_course',
    fromMap: ZStudyProgramCourse.fromMap,
    toMap: (value) => value.toMap(),
    fieldSpecs: $ZStudyProgramCourseFieldSpecs,
    fromMapWithContext: (map, context) => ZStudyProgramCourse.fromMap(
      map,
      extensionParser: context?.extensionParser == null
          ? null
          : (json) => context!.extensionParser!('study_program_course', json),
    ),
  );
}
...
const Set<String> $ZStudyProgramCourseTimestampFields = <String>{};
