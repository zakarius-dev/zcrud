...
part of 'z_study_workspace.dart';
...
ZStudyWorkspace _$ZStudyWorkspaceFromMap(Map<String, dynamic> map) =>
    ZStudyWorkspace(
      ...
      ownerPrincipalId: map['owner_principal_id'] is String
          ? map['owner_principal_id'] as String
          : null,
    );

extension ZStudyWorkspaceZcrud on ZStudyWorkspace {
...
const List<ZFieldSpec> $ZStudyWorkspaceFieldSpecs = <ZFieldSpec>[
  ...
  ZFieldSpec(name: 'owner_principal_id', type: EditionFieldType.text),
];

/// Clés que `ZStudyWorkspace.toMap()` PEUT produire — surensemble
...
const Set<String> $ZStudyWorkspacePersistedKeys = <String>{
  'id',
  'kind',
  'label',
  'owner_principal_id',
...
/// Enregistre `ZStudyWorkspace` (kind "study_workspace") sur [registry] : (dé)sérialisation + schéma.
void registerZStudyWorkspace(ZcrudRegistry registry) {
  // DW-ES14-1 (AD-4) : POUVOIR observé, pas seulement signature vérifiée.
  _$zRequireExtraPreserved<ZStudyWorkspace>(
    'ZStudyWorkspace',
    ZStudyWorkspace.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZStudyWorkspace>(
    'study_workspace',
    fromMap: ZStudyWorkspace.fromMap,
    toMap: (value) => value.toMap(),
    fieldSpecs: $ZStudyWorkspaceFieldSpecs,
    fromMapWithContext: (map, context) => ZStudyWorkspace.fromMap(
      map,
      extensionParser: context?.extensionParser == null
          ? null
          : (json) => context!.extensionParser!('study_workspace', json),
    ),
  );
}
...
const Set<String> $ZStudyWorkspaceTimestampFields = <String>{};
