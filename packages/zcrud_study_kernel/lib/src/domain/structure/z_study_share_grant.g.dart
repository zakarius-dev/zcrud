...
part of 'z_study_share_grant.dart';
...
ZStudyShareGrant _$ZStudyShareGrantFromMap(Map<String, dynamic> map) =>
    ZStudyShareGrant(
      id: map['id'] is String ? map['id'] as String : null,
      accessKey: map['access_key'] is String ? map['access_key'] as String : '',
      validFrom: _$asDateTime(map['valid_from']),
      validTo: _$asDateTime(map['valid_to']),
    );

extension ZStudyShareGrantZcrud on ZStudyShareGrant {
...
const Set<String> $ZStudyShareGrantPersistedKeys = <String>{
  'id',
  'access_key',
  'valid_from',
  'valid_to',
...
/// Enregistre `ZStudyShareGrant` (kind "study_share_grant") sur [registry] : (dé)sérialisation + schéma.
void registerZStudyShareGrant(ZcrudRegistry registry) {
  // DW-ES14-1 (AD-4) : POUVOIR observé, pas seulement signature vérifiée.
  _$zRequireExtraPreserved<ZStudyShareGrant>(
    'ZStudyShareGrant',
    ZStudyShareGrant.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZStudyShareGrant>(
    'study_share_grant',
    fromMap: ZStudyShareGrant.fromMap,
    toMap: (value) => value.toMap(),
    fieldSpecs: $ZStudyShareGrantFieldSpecs,
    fromMapWithContext: (map, context) => ZStudyShareGrant.fromMap(
      map,
      extensionParser: context?.extensionParser == null
          ? null
          : (json) => context!.extensionParser!('study_share_grant', json),
    ),
  );
}
...
const Set<String> $ZStudyShareGrantTimestampFields = <String>{};
