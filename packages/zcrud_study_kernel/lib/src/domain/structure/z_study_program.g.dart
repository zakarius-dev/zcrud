...
ZStudyProgram _$ZStudyProgramFromMap(Map<String, dynamic> map) => ZStudyProgram(
  ...
  credentialKind: map['credential_kind'] is String
      ? map['credential_kind'] as String
      : null,
  duration: map['duration'] is String ? map['duration'] as String : null,
  ancestorIds: map['ancestor_ids'] is List
      ? (map['ancestor_ids'] as List).whereType<String>().toList()
      : const <String>[],
);

extension ZStudyProgramZcrud on ZStudyProgram {
...
const List<ZFieldSpec> $ZStudyProgramFieldSpecs = <ZFieldSpec>[
  ...
  ZFieldSpec(name: 'credential_kind', type: EditionFieldType.text),
  ZFieldSpec(name: 'duration', type: EditionFieldType.text),
  ZFieldSpec(name: 'ancestor_ids', type: EditionFieldType.text, multiple: true),
];

/// Clés que `ZStudyProgram.toMap()` PEUT produire — surensemble
...
const Set<String> $ZStudyProgramPersistedKeys = <String>{
  'id',
  'organization_id',
  'parent_id',
  'kind',
...
/// Enregistre `ZStudyProgram` (kind "study_program") sur [registry] : (dé)sérialisation + schéma.
void registerZStudyProgram(ZcrudRegistry registry) {
  // DW-ES14-1 (AD-4) : POUVOIR observé, pas seulement signature vérifiée.
  _$zRequireExtraPreserved<ZStudyProgram>(
    'ZStudyProgram',
    ZStudyProgram.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZStudyProgram>(
    'study_program',
    fromMap: ZStudyProgram.fromMap,
    toMap: (value) => value.toMap(),
    fieldSpecs: $ZStudyProgramFieldSpecs,
    fromMapWithContext: (map, context) => ZStudyProgram.fromMap(
      map,
      extensionParser: context?.extensionParser == null
          ? null
          : (json) => context!.extensionParser!('study_program', json),
    ),
  );
}
...
const Set<String> $ZStudyProgramTimestampFields = <String>{};
