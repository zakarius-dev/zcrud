...
part of 'z_study_classification.dart';
...
ZStudyClassification _$ZStudyClassificationFromMap(Map<String, dynamic> map) =>
    ZStudyClassification(
      id: map['id'] is String ? map['id'] as String : null,
      vocabularyKey: map['vocabulary_key'] is String
          ? map['vocabulary_key'] as String
          : '',
      valueKey: map['value_key'] is String ? map['value_key'] as String : '',
...
extension ZStudyClassificationZcrud on ZStudyClassification {
...
const List<ZFieldSpec> $ZStudyClassificationFieldSpecs = <ZFieldSpec>[
  ...
  ZFieldSpec(name: 'vocabulary_key', type: EditionFieldType.text),
  ZFieldSpec(name: 'value_key', type: EditionFieldType.text),
...
/// Clés que `ZStudyClassification.toMap()` PEUT produire — surensemble
...
const Set<String> $ZStudyClassificationPersistedKeys = <String>{
  'id',
  'vocabulary_key',
  'value_key',
  'period_id',
...
/// Enregistre `ZStudyClassification` (kind "study_classification") sur [registry] : (dé)sérialisation + schéma.
void registerZStudyClassification(ZcrudRegistry registry) {
  // DW-ES14-1 (AD-4) : POUVOIR observé, pas seulement signature vérifiée.
  _$zRequireExtraPreserved<ZStudyClassification>(
    'ZStudyClassification',
    ZStudyClassification.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZStudyClassification>(
    'study_classification',
    fromMap: ZStudyClassification.fromMap,
    toMap: (value) => value.toMap(),
    fieldSpecs: $ZStudyClassificationFieldSpecs,
    fromMapWithContext: (map, context) => ZStudyClassification.fromMap(
      map,
      extensionParser: context?.extensionParser == null
          ? null
          : (json) => context!.extensionParser!('study_classification', json),
    ),
  );
}
...
const Set<String> $ZStudyClassificationTimestampFields = <String>{};
