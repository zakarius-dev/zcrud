...
part of 'z_study_competency.dart';
...
ZStudyCompetency _$ZStudyCompetencyFromMap(Map<String, dynamic> map) =>
    ZStudyCompetency(
      id: map['id'] is String ? map['id'] as String : null,
      frameworkId: map['framework_id'] is String
          ? map['framework_id'] as String
          : '',
...
extension ZStudyCompetencyZcrud on ZStudyCompetency {
...
const List<ZFieldSpec> $ZStudyCompetencyFieldSpecs = <ZFieldSpec>[
  ...
  ZFieldSpec(name: 'framework_id', type: EditionFieldType.text),
  ZFieldSpec(name: 'code', type: EditionFieldType.text),
  ZFieldSpec(name: 'label', type: EditionFieldType.text),
  ZFieldSpec(name: 'description', type: EditionFieldType.text),
];

/// Clés que `ZStudyCompetency.toMap()` PEUT produire — surensemble
...
const Set<String> $ZStudyCompetencyPersistedKeys = <String>{
  'id',
  'framework_id',
  'code',
  'label',
...
/// Enregistre `ZStudyCompetency` (kind "study_competency") sur [registry] : (dé)sérialisation + schéma.
void registerZStudyCompetency(ZcrudRegistry registry) {
  // DW-ES14-1 (AD-4) : POUVOIR observé, pas seulement signature vérifiée.
  _$zRequireExtraPreserved<ZStudyCompetency>(
    'ZStudyCompetency',
    ZStudyCompetency.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZStudyCompetency>(
    'study_competency',
    fromMap: ZStudyCompetency.fromMap,
    toMap: (value) => value.toMap(),
    fieldSpecs: $ZStudyCompetencyFieldSpecs,
    fromMapWithContext: (map, context) => ZStudyCompetency.fromMap(
      map,
      extensionParser: context?.extensionParser == null
          ? null
          : (json) => context!.extensionParser!('study_competency', json),
    ),
  );
}
...
const Set<String> $ZStudyCompetencyTimestampFields = <String>{};
