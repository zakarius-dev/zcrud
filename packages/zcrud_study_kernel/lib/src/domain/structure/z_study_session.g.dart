...
ZStudySession _$ZStudySessionFromMap(Map<String, dynamic> map) => ZStudySession(
  id: map['id'] is String ? map['id'] as String : null,
  offeringId: map['offering_id'] is String ? map['offering_id'] as String : '',
  startsAt: _$asDateTime(map['starts_at']),
  endsAt: _$asDateTime(map['ends_at']),
  kind: map['kind'] is String ? map['kind'] as String : '',
  meetingUrl: map['meeting_url'] is String
      ? map['meeting_url'] as String
      : null,
);

extension ZStudySessionZcrud on ZStudySession {
...
/// `toMap()`/`copyWith()` de `ZStudySession` en MEMBRES D'INSTANCE.
///
/// À appliquer (`class ZStudySession … with _$ZStudySessionZcrud`) quand un membre
...
const List<ZFieldSpec> $ZStudySessionFieldSpecs = <ZFieldSpec>[
  ZFieldSpec(name: 'id', type: EditionFieldType.text, isId: true),
  ZFieldSpec(name: 'offering_id', type: EditionFieldType.text),
  ZFieldSpec(name: 'starts_at', type: EditionFieldType.dateTime),
  ZFieldSpec(name: 'ends_at', type: EditionFieldType.dateTime),
  ZFieldSpec(name: 'kind', type: EditionFieldType.text),
  ZFieldSpec(name: 'meeting_url', type: EditionFieldType.text),
];

/// Clés que `ZStudySession.toMap()` PEUT produire — surensemble
...
const Set<String> $ZStudySessionPersistedKeys = <String>{
  'id',
  'offering_id',
  'starts_at',
  'ends_at',
...
/// Enregistre `ZStudySession` (kind "study_session") sur [registry] : (dé)sérialisation + schéma.
void registerZStudySession(ZcrudRegistry registry) {
  // DW-ES14-1 (AD-4) : POUVOIR observé, pas seulement signature vérifiée.
  _$zRequireExtraPreserved<ZStudySession>(
    'ZStudySession',
    ZStudySession.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZStudySession>(
    'study_session',
    fromMap: ZStudySession.fromMap,
    toMap: (value) => value.toMap(),
    fieldSpecs: $ZStudySessionFieldSpecs,
    fromMapWithContext: (map, context) => ZStudySession.fromMap(
      map,
      extensionParser: context?.extensionParser == null
          ? null
          : (json) => context!.extensionParser!('study_session', json),
    ),
  );
}
...
const Set<String> $ZStudySessionTimestampFields = <String>{};
