...
part of 'z_study_offering_audience.dart';
...
ZStudyOfferingAudience _$ZStudyOfferingAudienceFromMap(
  ...
  groupId: map['group_id'] is String ? map['group_id'] as String : '',
  role: map['role'] is String ? map['role'] as String : null,
);

extension ZStudyOfferingAudienceZcrud on ZStudyOfferingAudience {
...
const List<ZFieldSpec> $ZStudyOfferingAudienceFieldSpecs = <ZFieldSpec>[
  ...
  ZFieldSpec(name: 'group_id', type: EditionFieldType.text),
  ZFieldSpec(name: 'role', type: EditionFieldType.text),
];

/// Clés que `ZStudyOfferingAudience.toMap()` PEUT produire — surensemble
...
const Set<String> $ZStudyOfferingAudiencePersistedKeys = <String>{
  'id',
  'offering_id',
  'group_id',
  'role',
...
/// Enregistre `ZStudyOfferingAudience` (kind "study_offering_audience") sur [registry] : (dé)sérialisation + schéma.
void registerZStudyOfferingAudience(ZcrudRegistry registry) {
  // DW-ES14-1 (AD-4) : POUVOIR observé, pas seulement signature vérifiée.
  _$zRequireExtraPreserved<ZStudyOfferingAudience>(
    'ZStudyOfferingAudience',
    ZStudyOfferingAudience.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZStudyOfferingAudience>(
    'study_offering_audience',
    fromMap: ZStudyOfferingAudience.fromMap,
    toMap: (value) => value.toMap(),
    fieldSpecs: $ZStudyOfferingAudienceFieldSpecs,
    fromMapWithContext: (map, context) => ZStudyOfferingAudience.fromMap(
      map,
      extensionParser: context?.extensionParser == null
          ? null
          : (json) =>
                context!.extensionParser!('study_offering_audience', json),
    ),
  );
}
...
const Set<String> $ZStudyOfferingAudienceTimestampFields = <String>{};
