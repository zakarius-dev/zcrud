...
part of 'z_study_participation.dart';
...
ZStudyParticipation _$ZStudyParticipationFromMap(Map<String, dynamic> map) =>
    ZStudyParticipation(
      id: map['id'] is String ? map['id'] as String : null,
      role: map['role'] is String ? map['role'] as String : '',
...
extension ZStudyParticipationZcrud on ZStudyParticipation {
...
/// Clés que `ZStudyParticipation.toMap()` PEUT produire — surensemble
...
const Set<String> $ZStudyParticipationPersistedKeys = <String>{
  'id',
  'role',
  'period_id',
  'valid_from',
...
/// Enregistre `ZStudyParticipation` (kind "study_participation") sur [registry] : (dé)sérialisation + schéma.
void registerZStudyParticipation(ZcrudRegistry registry) {
  // DW-ES14-1 (AD-4) : POUVOIR observé, pas seulement signature vérifiée.
  _$zRequireExtraPreserved<ZStudyParticipation>(
    'ZStudyParticipation',
    ZStudyParticipation.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZStudyParticipation>(
    'study_participation',
    fromMap: ZStudyParticipation.fromMap,
    toMap: (value) => value.toMap(),
    fieldSpecs: $ZStudyParticipationFieldSpecs,
    fromMapWithContext: (map, context) => ZStudyParticipation.fromMap(
      map,
      extensionParser: context?.extensionParser == null
          ? null
          : (json) => context!.extensionParser!('study_participation', json),
    ),
  );
}
...
const Set<String> $ZStudyParticipationTimestampFields = <String>{};
