...
ZStudyTopic _$ZStudyTopicFromMap(Map<String, dynamic> map) => ZStudyTopic(
  id: map['id'] is String ? map['id'] as String : null,
  curriculumId: map['curriculum_id'] is String
  ...
  expectedDuration: map['expected_duration'] is String
      ? map['expected_duration'] as String
      : null,
  weight: _$asDouble(map['weight']),
  ancestorIds: map['ancestor_ids'] is List
      ? (map['ancestor_ids'] as List).whereType<String>().toList()
      : const <String>[],
);

extension ZStudyTopicZcrud on ZStudyTopic {
...
/// `toMap()`/`copyWith()` de `ZStudyTopic` en MEMBRES D'INSTANCE.
///
/// À appliquer (`class ZStudyTopic … with _$ZStudyTopicZcrud`) quand un membre
...
const List<ZFieldSpec> $ZStudyTopicFieldSpecs = <ZFieldSpec>[
  ZFieldSpec(name: 'id', type: EditionFieldType.text, isId: true),
  ZFieldSpec(name: 'curriculum_id', type: EditionFieldType.text),
  ...
  ZFieldSpec(name: 'expected_duration', type: EditionFieldType.text),
  ZFieldSpec(name: 'weight', type: EditionFieldType.float),
  ZFieldSpec(name: 'ancestor_ids', type: EditionFieldType.text, multiple: true),
];

/// Clés que `ZStudyTopic.toMap()` PEUT produire — surensemble
...
const Set<String> $ZStudyTopicPersistedKeys = <String>{
  'id',
  'curriculum_id',
  'parent_id',
  'kind',
...
/// Enregistre `ZStudyTopic` (kind "study_topic") sur [registry] : (dé)sérialisation + schéma.
void registerZStudyTopic(ZcrudRegistry registry) {
  // DW-ES14-1 (AD-4) : POUVOIR observé, pas seulement signature vérifiée.
  _$zRequireExtraPreserved<ZStudyTopic>(
    'ZStudyTopic',
    ZStudyTopic.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZStudyTopic>(
    'study_topic',
    fromMap: ZStudyTopic.fromMap,
    toMap: (value) => value.toMap(),
    fieldSpecs: $ZStudyTopicFieldSpecs,
    fromMapWithContext: (map, context) => ZStudyTopic.fromMap(
      map,
      extensionParser: context?.extensionParser == null
          ? null
          : (json) => context!.extensionParser!('study_topic', json),
    ),
  );
}
...
const Set<String> $ZStudyTopicTimestampFields = <String>{};
