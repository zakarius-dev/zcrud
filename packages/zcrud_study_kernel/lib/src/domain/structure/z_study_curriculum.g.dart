...
part of 'z_study_curriculum.dart';
...
ZStudyCurriculum _$ZStudyCurriculumFromMap(
  ...
  courseId: map['course_id'] is String ? map['course_id'] as String : null,
  programId: map['program_id'] is String ? map['program_id'] as String : null,
  ...
  effectiveFrom: _$asDateTime(map['effective_from']),
  effectiveTo: _$asDateTime(map['effective_to']),
);

extension ZStudyCurriculumZcrud on ZStudyCurriculum {
...
const List<ZFieldSpec> $ZStudyCurriculumFieldSpecs = <ZFieldSpec>[
  ...
  ZFieldSpec(name: 'subject_id', type: EditionFieldType.text),
  ...
  ZFieldSpec(name: 'effective_from', type: EditionFieldType.dateTime),
  ZFieldSpec(name: 'effective_to', type: EditionFieldType.dateTime),
];

/// Clés que `ZStudyCurriculum.toMap()` PEUT produire — surensemble
...
const Set<String> $ZStudyCurriculumPersistedKeys = <String>{
  'id',
  'organization_id',
  'subject_id',
  'course_id',
...
/// Enregistre `ZStudyCurriculum` (kind "study_curriculum") sur [registry] : (dé)sérialisation + schéma.
void registerZStudyCurriculum(ZcrudRegistry registry) {
  // DW-ES14-1 (AD-4) : POUVOIR observé, pas seulement signature vérifiée.
  _$zRequireExtraPreserved<ZStudyCurriculum>(
    'ZStudyCurriculum',
    ZStudyCurriculum.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZStudyCurriculum>(
    'study_curriculum',
    fromMap: ZStudyCurriculum.fromMap,
    toMap: (value) => value.toMap(),
    fieldSpecs: $ZStudyCurriculumFieldSpecs,
    fromMapWithContext: (map, context) => ZStudyCurriculum.fromMap(
      map,
      extensionParser: context?.extensionParser == null
          ? null
          : (json) => context!.extensionParser!('study_curriculum', json),
    ),
  );
}
...
const Set<String> $ZStudyCurriculumTimestampFields = <String>{};
