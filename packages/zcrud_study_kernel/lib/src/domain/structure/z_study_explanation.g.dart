...
part of 'z_study_explanation.dart';
...
ZStudyExplanation _$ZStudyExplanationFromMap(Map<String, dynamic> map) =>
    ZStudyExplanation(
      ...
      relatedTopics: map['related_topics'] is List
          ? (map['related_topics'] as List).whereType<String>().toList()
          : const <String>[],
      createdAt: _$asDateTime(map['created_at']),
    );

extension ZStudyExplanationZcrud on ZStudyExplanation {
...
const List<ZFieldSpec> $ZStudyExplanationFieldSpecs = <ZFieldSpec>[
  ...
  ZFieldSpec(name: 'content', type: EditionFieldType.text),
  ZFieldSpec(name: 'style', type: EditionFieldType.text),
  ZFieldSpec(name: 'operation', type: EditionFieldType.text),
  ZFieldSpec(
    name: 'related_topics',
...
/// Clés que `ZStudyExplanation.toMap()` PEUT produire — surensemble
...
const Set<String> $ZStudyExplanationPersistedKeys = <String>{
  'id',
  'folder_id',
  'content',
  'style',
...
/// Enregistre `ZStudyExplanation` (kind "study_explanation") sur [registry] : (dé)sérialisation + schéma.
void registerZStudyExplanation(ZcrudRegistry registry) {
  // DW-ES14-1 (AD-4) : POUVOIR observé, pas seulement signature vérifiée.
  _$zRequireExtraPreserved<ZStudyExplanation>(
    'ZStudyExplanation',
    ZStudyExplanation.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZStudyExplanation>(
    'study_explanation',
    fromMap: ZStudyExplanation.fromMap,
    toMap: (value) => value.toMap(),
    fieldSpecs: $ZStudyExplanationFieldSpecs,
    fromMapWithContext: (map, context) => ZStudyExplanation.fromMap(
      map,
      extensionParser: context?.extensionParser == null
          ? null
          : (json) => context!.extensionParser!('study_explanation', json),
    ),
  );
}
...
const Set<String> $ZStudyExplanationTimestampFields = <String>{};
