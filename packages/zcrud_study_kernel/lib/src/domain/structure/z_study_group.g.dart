...
ZStudyGroup _$ZStudyGroupFromMap(Map<String, dynamic> map) => ZStudyGroup(
  id: map['id'] is String ? map['id'] as String : null,
  organizationId: map['organization_id'] is String
      ? map['organization_id'] as String
      : null,
  parentGroupId: map['parent_group_id'] is String
      ? map['parent_group_id'] as String
      : null,
...
extension ZStudyGroupZcrud on ZStudyGroup {
...
/// `toMap()`/`copyWith()` de `ZStudyGroup` en MEMBRES D'INSTANCE.
///
/// À appliquer (`class ZStudyGroup … with _$ZStudyGroupZcrud`) quand un membre
...
/// Clés que `ZStudyGroup.toMap()` PEUT produire — surensemble
...
const Set<String> $ZStudyGroupPersistedKeys = <String>{
  'id',
  'organization_id',
  'parent_group_id',
  'kind',
...
/// Enregistre `ZStudyGroup` (kind "study_group") sur [registry] : (dé)sérialisation + schéma.
void registerZStudyGroup(ZcrudRegistry registry) {
  // DW-ES14-1 (AD-4) : POUVOIR observé, pas seulement signature vérifiée.
  _$zRequireExtraPreserved<ZStudyGroup>(
    'ZStudyGroup',
    ZStudyGroup.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZStudyGroup>(
    'study_group',
    fromMap: ZStudyGroup.fromMap,
    toMap: (value) => value.toMap(),
    fieldSpecs: $ZStudyGroupFieldSpecs,
    fromMapWithContext: (map, context) => ZStudyGroup.fromMap(
      map,
      extensionParser: context?.extensionParser == null
          ? null
          : (json) => context!.extensionParser!('study_group', json),
    ),
  );
}
...
const Set<String> $ZStudyGroupTimestampFields = <String>{};
