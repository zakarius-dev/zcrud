...
ZStudyOffering _$ZStudyOfferingFromMap(Map<String, dynamic> map) =>
    ZStudyOffering(
...
/// Clés que `ZStudyOffering.toMap()` PEUT produire — surensemble
...
const Set<String> $ZStudyOfferingPersistedKeys = <String>{
  'id',
  'organization_id',
  'course_id',
  'period_id',
...
/// Enregistre `ZStudyOffering` (kind "study_offering") sur [registry] : (dé)sérialisation + schéma.
void registerZStudyOffering(ZcrudRegistry registry) {
  // DW-ES14-1 (AD-4) : POUVOIR observé, pas seulement signature vérifiée.
  _$zRequireExtraPreserved<ZStudyOffering>(
    'ZStudyOffering',
    ZStudyOffering.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZStudyOffering>(
    'study_offering',
    fromMap: ZStudyOffering.fromMap,
    toMap: (value) => value.toMap(),
    fieldSpecs: $ZStudyOfferingFieldSpecs,
    fromMapWithContext: (map, context) => ZStudyOffering.fromMap(
      map,
      extensionParser: context?.extensionParser == null
          ? null
          : (json) => context!.extensionParser!('study_offering', json),
    ),
  );
}
...
const Set<String> $ZStudyOfferingTimestampFields = <String>{};
