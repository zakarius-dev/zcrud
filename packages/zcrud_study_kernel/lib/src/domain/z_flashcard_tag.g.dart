...
ZFlashcardTag _$ZFlashcardTagFromMap(Map<String, dynamic> map) => ZFlashcardTag(
...
extension ZFlashcardTagZcrud on ZFlashcardTag {
...
/// `toMap()`/`copyWith()` de `ZFlashcardTag` en MEMBRES D'INSTANCE.
///
/// À appliquer (`class ZFlashcardTag … with _$ZFlashcardTagZcrud`) quand un membre
...
const List<ZFieldSpec> $ZFlashcardTagFieldSpecs = <ZFieldSpec>[
  ZFieldSpec(name: 'id', type: EditionFieldType.text, isId: true),
  ZFieldSpec(name: 'title', type: EditionFieldType.text, label: 'Tag'),
  ZFieldSpec(name: 'color_key', type: EditionFieldType.text),
];

/// Clés que `ZFlashcardTag.toMap()` PEUT produire — surensemble
...
void registerZFlashcardTag(ZcrudRegistry registry) {
  ...
  _$zRequireExtraPreserved<ZFlashcardTag>(
    'ZFlashcardTag',
    ZFlashcardTag.fromMap,
    (value) => value.toMap(),
    (value) => value.extra,
  );
  registry.register<ZFlashcardTag>(
    'flashcard_tag',
...
const Set<String> $ZFlashcardTagTimestampFields = <String>{};
